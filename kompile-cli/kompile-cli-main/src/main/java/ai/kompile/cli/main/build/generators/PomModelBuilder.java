...
public class PomModelBuilder {
    ...
    private void addDependencies() {
        ...
        addDep(deps, "net.bytebuddy", "byte-buddy", "${byte-buddy.version}");
        ...
        for (ModuleCatalog.ModuleEntry entry : modules.getAllEntries()) {
            Dependency moduleDep = new Dependency();
            moduleDep.setGroupId(entry.getGroupId());
            moduleDep.setArtifactId(entry.getArtifactId());
            moduleDep.setVersion("${kompile.project.version}");
            // Exclude old BouncyCastle jdk15on — conflicts with jdk18on signer in native image
            for (String bcArtifact : new String[]{"bcprov-jdk15on", "bcmail-jdk15on", "bcpkix-jdk15on", "bcutil-jdk15on"}) {
                Exclusion ex = new Exclusion();
                ex.setGroupId("org.bouncycastle");
                ex.setArtifactId(bcArtifact);
                moduleDep.addExclusion(ex);
            }
            deps.add(moduleDep);
        }
    ...
    /**
     * Add a 'dist' profile with maven-assembly-plugin that packages the native binary
     * (or fat JAR), launcher scripts, config, and data scaffolding into zip + tar.gz.
     * The assembly descriptor is generated by {@link AppDistAssemblyGenerator}.
     */
    private void addDistProfile() {
        Profile distProfile = new Profile();
        distProfile.setId("dist");
        Build distBuild = new Build();

        Plugin assemblyPlugin = createPlugin("org.apache.maven.plugins",
                "maven-assembly-plugin", "3.7.1");

        Xpp3Dom assemblyConfig = new Xpp3Dom("configuration");
        Xpp3Dom descriptors = new Xpp3Dom("descriptors");
        Xpp3Dom descriptor = new Xpp3Dom("descriptor");
        descriptor.setValue("src/main/assembly/app-dist.xml");
        descriptors.addChild(descriptor);
        assemblyConfig.addChild(descriptors);
        addChild(assemblyConfig, "appendAssemblyId", "true");
        addChild(assemblyConfig, "attach", "false");
        assemblyPlugin.setConfiguration(assemblyConfig);

        PluginExecution distExec = new PluginExecution();
        distExec.setId("make-distribution");
        distExec.setPhase("package");
        distExec.addGoal("single");
        assemblyPlugin.addExecution(distExec);

        distBuild.addPlugin(assemblyPlugin);
        distProfile.setBuild(distBuild);
        model.addProfile(distProfile);
    }
