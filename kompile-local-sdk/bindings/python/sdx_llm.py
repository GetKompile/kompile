...
def load_library(path: Optional[str] = None) -> ctypes.CDLL:
    ...
    # CHANGE 2026-07-12 (coexistence fix): load with RTLD_LOCAL (ctypes default,
    # mode=0) instead of RTLD_GLOBAL.  Both libsdx_llm and libkompile_reasoning
    # embed a full GraalVM runtime that exports the same graal_* / JNI_* symbols.
    # When the first library is loaded RTLD_GLOBAL those symbols pollute the global
    # namespace and the second library's isolate init calls the wrong runtime,
    # producing ExceptionInInitializerError at sdxLlmLoadModel.  RTLD_LOCAL keeps
    # each library's symbols scoped to the handle returned by dlopen — both runtimes
    # remain completely independent.  koffi (TypeScript) and Rust's #[link] are
    # already handle-scoped by design; only the Python binding had this wrong.
    if lib_path is not None:
        _lib = ctypes.CDLL(lib_path)   # RTLD_LOCAL (default, mode=0)
    else:
        # System-installed fallback (LD_LIBRARY_PATH / ldconfig must find it).
        _lib = ctypes.CDLL("libsdx_llm.so")   # RTLD_LOCAL
...
def _ensure_native_lib_dir() -> None:
    ...
    libsdx_llm.so resolves its bundled native libraries (ND4J backend, BLAS,
    tokenizers, …) relative to the host executable, not the .so itself.
    Setting SDX_NATIVE_LIB_DIR overrides that lookup to point at the directory
    that actually contains the bundled .so files.

    CHANGE 2026-07-12 (coexistence fix): the Kompile SDK ships libsdx_llm.so
    in its own lib/ directory but that directory does NOT contain the ND4J
    side-libs (libjnind4jcpu.so, libjniopenblas.so, libjnitokenizers.so, …).
    Those live in the aot-sdk package's lib/ directory, which is either:
      a) $SDX_LLM_AOT_HOME/lib  — when the caller sets SDX_LLM_AOT_HOME to the
         unpacked aot-sdk root (preferred; $SDX_LLM_AOT_HOME/lib/libsdx_llm.so
         also lives there), or
      b) the directory that actually contains the loaded libsdx_llm.so — i.e.
         the same directory as the .so file itself (only valid when libsdx_llm.so
         was copied together with its side-libs, as in the full aot-sdk layout).
    Pointing at the Kompile SDK lib/ (which only has the .so files) causes
    ExceptionInInitializerError because the bundled JNI natives are absent.
    """
    ...
    if home:
        ...
        return
    # Fallback: use the directory of the loaded libsdx_llm.so (only valid when
    # the full aot-sdk layout was preserved — side-libs co-located with the .so).
    global _lib
    if _lib is not None:
        try:
            lib_path = pathlib.Path(_lib._name).resolve()
            if lib_path.is_file():
                os.environ["SDX_NATIVE_LIB_DIR"] = str(lib_path.parent)
        except Exception:
            pass
