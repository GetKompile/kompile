...
class _KgrLib:
    ...
    def __init__(self, lib_path: str):
        ...
        """
        # RTLD_LOCAL (ctypes default, mode=0) — DO NOT change to RTLD_GLOBAL.
        # libkompile_reasoning exports graal_* / JNI_* / __svm_* symbols that
        # are IDENTICAL in name to those exported by libsdx_llm (both are full
        # GraalVM native-image runtimes).  RTLD_GLOBAL would pollute the process
        # global namespace so that the second library's isolate bootstrap calls
        # (internal dlsym(RTLD_DEFAULT, "graal_create_isolate") etc.) bind to
        # the FIRST library's runtime, corrupting the second isolate's state and
        # producing ExceptionInInitializerError.  RTLD_LOCAL keeps each library's
        # runtime symbols scoped to the handle returned by dlopen — both runtimes
        # stay completely independent.  libkompile_reasoning also applies a GNU
        # version-script (kgr_exports.lds) at build time to hide graal_* from
        # .dynsym entirely, giving a second layer of defence.
        self._lib = ctypes.CDLL(lib_path)   # RTLD_LOCAL (default, mode=0)
