...
@Slf4j
public class KClawWebSocketHandler extends TextWebSocketHandler {
    ...
    public KClawWebSocketHandler(KClawAgentService agentService, ObjectMapper objectMapper) {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        try {
            ...
            KClawResponse response = agentService.execute(kClawRequest);
    ...
    private void sendResponse(WebSocketSession session, KClawResponse response) throws IOException {
        String json = objectMapper.writeValueAsString(response);
        session.sendMessage(new TextMessage(json));
    }

    private void sendError(WebSocketSession session, String error) throws IOException {
        KClawResponse response = KClawResponse.error(error);
        sendResponse(session, response);
    }
}
