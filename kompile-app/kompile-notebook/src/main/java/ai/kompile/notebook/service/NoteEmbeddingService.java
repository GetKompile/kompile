 ...
 * the Phase 1 plan ("anything text-based produced by the system must flow
 ...
 * This is intentionally simple — Phase 2+ can migrate to pgvector or a dedicated
 * embedding store if needed.
 */
@Service
public class NoteEmbeddingService {
     ...
     * Concatenates title (if present) and content for richer embedding context.
     */
    ...
    /**
     * Serialize an INDArray row vector to a comma-separated float string.
     * Uses the same approach as the Anserini vector store utilities.
     */
     ...
     * Exposed for potential use by search/retrieval components in Phase 2+.
     */
