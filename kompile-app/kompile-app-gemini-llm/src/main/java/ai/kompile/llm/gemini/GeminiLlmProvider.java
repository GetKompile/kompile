...
/**
 * Google Gemini LlmProvider implementation that supports model listing.
 * Uses the Gemini Developer API to list available models.
 */
@Component
...
public class GeminiLlmProvider implements LlmProvider {
    ...
    public int getMaxTokens() {
        return 1000000; // Gemini 1.5 Pro supports 1M context
    }
    ...
    public List<ModelInfo> getAvailableModels() {
        ...
        try {
            ...
            ResponseEntity<GeminiModelsResponse> response = restTemplate.exchange(
                    url,
                    HttpMethod.GET,
                    new HttpEntity<>(new HttpHeaders()),
                    GeminiModelsResponse.class
            );

            if (response.getBody() == null || response.getBody().models == null) {
                logger.warn("Gemini models API returned empty response");
                return getStaticModelList();
            }

            List<ModelInfo> models = new ArrayList<>();
            for (GeminiModel model : response.getBody().models) {
                // Only include generative models (not embedding models)
                if (model.name != null && model.name.contains("gemini")) {
                    ...
                    models.add(new ModelInfo(
                            ...
                            true // Gemini models support function calling
                    ));
                }
            }
            ...
            logger.debug("Retrieved {} Gemini models from API", models.size());
            return models;

        } catch (Exception e) {
            logger.error("Failed to fetch models from Gemini API: {}", e.getMessage());
            return getStaticModelList();
        }
    }

    private List<ModelInfo> getStaticModelList() {
        // Return known Gemini models when API key is not available
        return List.of(
                new ModelInfo("gemini-2.0-flash-exp", "Gemini 2.0 Flash", "Latest experimental flash model", 1000000, true),
                new ModelInfo("gemini-1.5-pro", "Gemini 1.5 Pro", "Most capable model with 1M context", 1000000, true),
                new ModelInfo("gemini-1.5-flash", "Gemini 1.5 Flash", "Fast and efficient", 1000000, true),
                new ModelInfo("gemini-1.5-flash-8b", "Gemini 1.5 Flash 8B", "Lightweight model", 1000000, true)
        );
    }
    ...
    public LlmSession startSession(LlmSessionRequest request) {
        throw new UnsupportedOperationException("Use GeminiLanguageModelImpl for chat sessions");
    }
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    private static class GeminiModel {
        @JsonProperty("name")
        public String name;
