...
public final class Constraints {
     ...
     */
    public static LogicalConstraint edgeOfType(String sourceVar, String targetVar, String edgeType) {
    ...
    /**
     * Constant FALSE constraint (never satisfied).
     *
     * <p>Used by the OWL 2 RL reasoner (Phase O2) as the consequent of consistency-checking
     * rules such as {@code cax-dw} (disjoint-class). When a grounding satisfies the antecedent
     * and the consequent is {@code falsehood()}, the {@link ai.kompile.graph.reasoning.fol.FolRule}
     * is violated and the OWL reasoner records an {@link ai.kompile.graph.reasoning.mebn.type.owl.OwlInconsistency}.
     * </p>
     *
     * <p>This is an additive factory method; it does not modify any existing constraint behaviour.</p>
     *
     * @return a {@link LogicalConstraint} that always evaluates to {@code false}
     */
    public static LogicalConstraint falsehood() {
        return new AtomicConstraint("FALSE", Set.of(), (kb, bindings) -> false);
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // TEMPORAL PREDICATES (Phase T2)
    // ═══════════════════════════════════════════════════════════════════════════

    /**
     * True when the entity bound to {@code xVar} has a {@link ai.kompile.graph.reasoning.model.GraphEntity#timestamp()}
     * that is strictly before the timestamp of the entity bound to {@code yVar}.
     *
     * <p>Resolution rules (mirrors the spec's section 5a precedence filter):
     * <ul>
     *   <li>Both timestamped → {@code x.timestamp.isBefore(y.timestamp)}.</li>
     *   <li>Either timestamp absent → returns {@code false} (cannot establish precedence).</li>
     * </ul>
     *
     * @param xVar variable name for the (candidate) earlier entity
     * @param yVar variable name for the (candidate) later entity
     * @return a {@link LogicalConstraint} that is true iff {@code x} temporally precedes {@code y}
     */
    public static LogicalConstraint precedes(String xVar, String yVar) {
        return new AtomicConstraint(
                "precedes(" + xVar + ", " + yVar + ")",
                Set.of(xVar, yVar),
                (kb, bindings) -> {
                    Optional<Instant> tx = kb.getTimestamp(bindings.get(xVar));
                    Optional<Instant> ty = kb.getTimestamp(bindings.get(yVar));
                    if (tx.isEmpty() || ty.isEmpty()) return false;
                    return tx.get().isBefore(ty.get());
                });
    }

    /**
     * True when the absolute duration between the timestamps of the entities bound to
     * {@code xVar} and {@code yVar} is at most {@code window}.
     *
     * <p>If either entity has no timestamp the constraint returns {@code false}.</p>
     *
     * @param xVar   variable name for the first entity
     * @param yVar   variable name for the second entity
     * @param window maximum allowed gap (inclusive); must not be {@code null}
     * @return a {@link LogicalConstraint} that is true iff {@code |time(x) − time(y)| ≤ window}
     */
    public static LogicalConstraint withinWindow(String xVar, String yVar, Duration window) {
        Objects.requireNonNull(window, "window");
        return new AtomicConstraint(
                "withinWindow(" + xVar + ", " + yVar + ", " + window + ")",
                Set.of(xVar, yVar),
                (kb, bindings) -> {
                    Optional<Instant> tx = kb.getTimestamp(bindings.get(xVar));
                    Optional<Instant> ty = kb.getTimestamp(bindings.get(yVar));
                    if (tx.isEmpty() || ty.isEmpty()) return false;
                    long diffMs = Math.abs(Duration.between(tx.get(), ty.get()).toMillis());
                    return diffMs <= window.toMillis();
                });
    }

    /**
     * True when the Allen interval relation between the valid-time intervals of the entities
     * bound to {@code xVar} and {@code yVar} is exactly {@code rel}.
     *
     * <p>Resolution rules:
     * <ul>
     *   <li>Both entities carry a {@link TemporalInterval} (via
     *       {@link ai.kompile.graph.reasoning.model.GraphEntity#validTime()}) → compute
     *       {@link AllenRelation#compute(TemporalInterval, TemporalInterval)} and compare.</li>
     *   <li>Either interval absent but a {@link ai.kompile.graph.reasoning.model.GraphEntity#timestamp()}
     *       is present → the point timestamp is wrapped as
     *       {@link TemporalInterval#point(Instant)} and used in its place.</li>
     *   <li>No temporal information available → returns {@code false}.</li>
     * </ul>
     *
     * @param xVar variable name for the first entity
     * @param yVar variable name for the second entity
     * @param rel  the required Allen relation
     * @return a {@link LogicalConstraint} that is true iff the entities' intervals satisfy {@code rel}
     */
    public static LogicalConstraint allenRelation(String xVar, String yVar, AllenRelation rel) {
        Objects.requireNonNull(rel, "rel");
        return new AtomicConstraint(
                "allenRelation(" + xVar + ", " + yVar + ", " + rel + ")",
                Set.of(xVar, yVar),
                (kb, bindings) -> {
                    TemporalInterval ix = resolveInterval(kb, bindings.get(xVar));
                    TemporalInterval iy = resolveInterval(kb, bindings.get(yVar));
                    if (ix == null || iy == null) return false;
                    return AllenRelation.compute(ix, iy) == rel;
                });
    }

    /**
     * Resolve a {@link TemporalInterval} for an entity from the knowledge base.
     * Tries {@code getValidTime()} first; falls back to wrapping {@code getTimestamp()} as a
     * point interval.
     *
     * @param kb       the knowledge base
     * @param entityId the entity id to look up
     * @return the interval, or {@code null} when no temporal information is available
     */
    private static TemporalInterval resolveInterval(KnowledgeBase kb, String entityId) {
        if (entityId == null) return null;
        Optional<TemporalInterval> vt = kb.getValidTime(entityId);
        if (vt.isPresent()) return vt.get();
        Optional<Instant> ts = kb.getTimestamp(entityId);
        return ts.map(TemporalInterval::point).orElse(null);
    }
