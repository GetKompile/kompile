...
public class AttributionChain {
    ...
    /**
     * Whether every hop in this chain is temporally ordered: the cause timestamp does not
     * come after the effect timestamp. A hop with {@code null} timestamps on either end is
     * treated as consistent (temporal unknown — cannot rule it out).
     *
     * <p>A chain that returns {@code false} violates the causal arrow direction — at least one
     * hop has a cause that post-dates its effect — and should be demoted to
     * {@link AttributionConfidence#INSUFFICIENT} or dropped.</p>
     *
     * @return {@code true} iff all hops satisfy {@code causeTimestamp <= effectTimestamp}
     */
