...
public class MFrag implements Serializable {
    ...
    /**
     * Default distribution: used during SSBN construction when no context constraint is satisfied
     * for a particular entity instantiation.  This implements the MEBN canonical concept of a
     * "default MFrag" distribution (Laskey 2008, Section 3.3): every resident RV must have a
     * designated fall-back CPD that is applied whenever the specific MFrag context does not apply.
     *
    ...
    /**
     * Set the default distribution used when context constraints are NOT satisfied.
     *
     * <p>Per canonical MEBN (Laskey 2008): when a particular entity grounding does not satisfy
     * this MFrag's context constraints, the SSBN generator falls back to this distribution
     * rather than leaving the RV without a CPT.  This allows the MTheory to define a sensible
     * prior for every grounding of every resident RV.</p>
     *
     * @param defaultDistribution function(rvName, parentStrengths) → CPT entries; or null for
     *                             a uniform prior fallback
     */
    public void setDefaultDistribution(BiFunction<String, double[], double[]> defaultDistribution) {
        this.defaultDistribution = defaultDistribution;
    }

    public BiFunction<String, double[], double[]> getDefaultDistribution() {
    ...
    /**
     * Update the causal strength of an existing parent edge in place (unlike {@link #addParentEdge}
     * it does not touch the parent structure) — used by noisy-OR weight learning to fit edge strengths
     * to observed data.
     */
    public MFrag setEdgeStrength(String parentRvName, String residentRvName, double strength) {
        edgeStrengths.put(parentRvName + "->" + residentRvName, strength);
        return this;
    }
