...
public class PipelineNativeEntryPoints {
    ...
    // .CEntryPointBuiltins).  The exported C symbol names follow the camelCase
    // naming convention of the existing pipeline ABI (kompile* prefix) so
    // consumers never need to include graal_isolate.h.
    //
    // Thread rule: one kompile_thread_t* per OS thread; handles are NOT shared.
    // Single-threaded: kompileCreateIsolate → all pipeline calls → kompileTearDownIsolate.
    // Additional threads: kompileAttachThread on entry, kompileDetachThread on exit.

    /**
     * Create a new GraalVM isolate and attach the calling OS thread.
     * Equivalent to {@code graal_create_isolate} but exported under the
     * {@code kompile*} namespace so consumers need only {@code kompile.h}.
     *
     * <p>C signature (generated by native-image):
     * <pre>
     *   int kompileCreateIsolate(void*                params,
     *                            kompile_isolate_t**  isolate,
     *                            kompile_thread_t**   thread);
     * </pre>
     *
     * @return 0 on success, non-zero on failure
     */
    @CEntryPoint(name = "kompileCreateIsolate",
                 builtin = CEntryPoint.Builtin.CREATE_ISOLATE)
    public static native IsolateThread kompileCreateIsolate();
     ...
     * before that thread makes any other pipeline calls.
     *
     * <p>C signature (generated by native-image):
     * <pre>
     *   int kompileAttachThread(kompile_isolate_t*  isolate,
     *                           kompile_thread_t**  thread);
     * </pre>
     *
     ...
     * @return 0 on success, non-zero on failure
     */
    @CEntryPoint(name = "kompileAttachThread",
                 builtin = CEntryPoint.Builtin.ATTACH_THREAD)
    public static native IsolateThread kompileAttachThread(Isolate isolate);
     ...
     * @return 0 on success, non-zero on failure
     */
    @CEntryPoint(name = "kompileDetachThread",
                 builtin = CEntryPoint.Builtin.DETACH_THREAD)
    public static native int kompileDetachThread(IsolateThread thread);

    /**
     * Tear down the isolate: releases all heap, detaches all threads.
     * After this call no pipeline function may be called until a new isolate
     * is created. The thread handle is also invalidated.
     *
     * <p>C signature (generated by native-image):
     * <pre>
     ...
     * @return 0 on success, non-zero on failure
     */
    @CEntryPoint(name = "kompileTearDownIsolate",
                 builtin = CEntryPoint.Builtin.TEAR_DOWN_ISOLATE)
    public static native int kompileTearDownIsolate(IsolateThread thread);

    // ── ABI version ──────────────────────────────────────────────────────────

    /**
     * Return the ABI version of this library build.
     * Consumers should check this at startup against {@code KOMPILE_ABI_VERSION}
     * in {@code kompile.h} to detect incompatible builds.
     *
     * <p>Current version: 1.  Bump only on breaking changes (removed symbols,
     * changed parameter types, changed semantics).  Additive changes do not bump.
     *
     * @param thread a valid attached thread handle
     * @return 1 (current ABI version)
     */
    @CEntryPoint(name = "kompileAbiVersion")
    public static int kompileAbiVersion(IsolateThread thread) {
        return 1;
    }
