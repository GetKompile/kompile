...
public class DL4JLanguageModelStepRunnerFactory implements PipelineStepRunnerFactory, StepSchemaProvider {

    // Matches uniqueTypeName() in DL4JLanguageModelStepRunner
    public static final String STEP_TYPE_NAME = "DL4J_LANGUAGE_MODEL";
    ...
    public PipelineStepRunner create() {
        return new DL4JLanguageModelStepRunner();
    }
    ...
    public StepSchema getSchema() {
        return StepSchema.builder()
                .name(stepTypeName())
                .description("Runs a Deeplearning4j (DL4J) based Language Model (ComputationGraph or SameDiff) for text generation and tool calling, using LLMStepConfig.")
                ...
                .parameters(Arrays.asList(
                        ...
                        // 'type' is implicitly the factory's stepTypeName
                        ...
                        ParameterSchema.builder().name("modelUri").type(ValueType.STRING).description("URI to the DL4J language model file (e.g., ComputationGraph or SameDiff model).").required(true).build(),
                        ...
                        ParameterSchema.builder().name("toolCallOutputFormat").type(ValueType.STRING).description("Expected model-owned tool-call protocol. OPENAI_JSON requires one complete JSON envelope; legacy JSON_MARKER_BASED is a strict JSON migration alias.").defaultValue(LLMStepConfig.ToolCallOutputFormat.OPENAI_JSON.name()).required(false).build(),
                        ParameterSchema.builder().name("generationParameters").type(ValueType.OBJECT).description("Map of generation parameters (e.g., temperature, maxNewTokens, topK).").type(ValueType.valueOf(Map.class.getName())).required(false).build()
                ))
