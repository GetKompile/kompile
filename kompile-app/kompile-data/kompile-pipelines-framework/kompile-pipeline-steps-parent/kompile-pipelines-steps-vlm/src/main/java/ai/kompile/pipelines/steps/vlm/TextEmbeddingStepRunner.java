...
/**
 * Runs the text embedding (embed_tokens) stage of a VLM pipeline.
 *
 * Input Data keys:
 *   - input_ids: NDArray [1, seq_len] of INT64
 *
 * Output Data keys:
 *   - text_embeddings: NDArray [1, seq_len, hidden_size]
 *
 * Config parameters:
 *   - modelUri: Path to the embed_tokens SameDiff model (.fb file)
 *   - outputNames: List of model output names
 */
public class TextEmbeddingStepRunner implements PipelineStepRunner {
    ...
    public void init(StepConfig stepConfig, Context context) throws Exception {
        ...
        Objects.requireNonNull(modelUri, "modelUri is required for TextEmbeddingStepRunner");
        ...
        if (this.outputNames == null || this.outputNames.isEmpty()) {
            throw new IllegalArgumentException("outputNames is required for TextEmbeddingStepRunner");
        }

        File modelFile = new File(modelUri);
        if (!modelFile.exists()) {
            throw new IllegalArgumentException("Text embedding model not found: " + modelUri);
        }
    ...
    public Data exec(Data input, Context context) throws Exception {
        if (!initialized) {
            throw new IllegalStateException("TextEmbeddingStepRunner not initialized");
        }
