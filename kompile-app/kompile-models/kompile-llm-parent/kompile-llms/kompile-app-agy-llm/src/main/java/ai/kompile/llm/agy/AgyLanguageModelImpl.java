...
@Service("agyLanguageModel")
@ConditionalOnClass(name = "ai.kompile.llm.agy.AgyLanguageModelImpl")
@ConditionalOnProperty(prefix = "spring.ai.vertex.ai.gemini", name = "project-id") // Condition on Vertex AI project ID
public class AgyLanguageModelImpl implements LanguageModel {

    private static final Logger logger = LoggerFactory.getLogger(AgyLanguageModelImpl.class);
    private final ChatClient chatClient;

    @Autowired
    public AgyLanguageModelImpl(ChatClient.Builder chatClientBuilder) {
        ...
        logger.info("AgyLanguageModelImpl initialized with ChatClient: {}", this.chatClient.getClass().getName());
    }

    @Override
    public String generateResponse(String userQuery, List<String> context) {
        logger.debug("Agy generating simple response for query: {}", userQuery);
    ...
    public ChatResponse generateResponseWithPotentialToolCalls(String userQuery, List<String> context) {
        logger.debug("Agy generating response with potential tool calls for query: {}", userQuery);
        ...
        logger.info("Advertising tools to Agy: {}", String.join(", ", toolNamesArray));
        ...
        logger.debug("Agy LLM response (may include tool call): {}", response);
        return response;
    }
}