...
public class KompilePipelineLanguageModelImpl implements LanguageModel {
    ...
    public String generateResponse(String userQuery, List<String> contextDokumente) {
        ...
        throw new IllegalStateException("Kompile pipeline did not produce a final textual response");
    }
    ...
    private ChatResponse orchestratePipelineExecution(
        ...
        try {
            ...
            if (toolIterationCount >= MAX_TOOL_ITERATIONS
                    && pipelineOutput != null
                    && pipelineOutput.getBoolean(PipelineDataConstants.IS_TOOL_CALL_REQUEST_FLAG_KEY, false)) {
                String lastToolRequests = toJsonQuietly(pipelineOutput.getList(PipelineDataConstants.TOOL_CALL_REQUESTS_KEY, ValueType.DATA));
                throw new IllegalStateException("Kompile pipeline exceeded max tool iterations. Last tool requests were: " + lastToolRequests);
            }
        ...
        if (pipelineOutput == null) { // Should not happen if loop executed at least once
            throw new IllegalStateException("Kompile pipeline produced null output");
        }
        if (pipelineOutput.getBoolean(PipelineDataConstants.IS_TOOL_CALL_REQUEST_FLAG_KEY, false)) {
            throw new IllegalStateException("Kompile pipeline ended while still requesting tool calls");
        }

        String responseText = pipelineOutput.getString(this.responseOutputName, "");
        if (responseText == null || responseText.isBlank()) {
            throw new IllegalStateException("Kompile pipeline did not produce required output field '" + this.responseOutputName + "'");
        }
    ...
    private PipelineToolCallResponse executeInternalTool(PipelineToolCallRequest toolRequest, Context toolContext) {
        ...
        if (toolPipelineOpt.isEmpty()) {
            throw new IllegalStateException("Internal tool pipeline '" + toolPipelineId + "' not found");
        }
        ...
        try {
            ...
            if (toolRequest.getArguments() != null && !toolRequest.getArguments().isEmpty()) {
                try {
                    ...
                    throw new IllegalArgumentException("Invalid arguments format for tool " + toolPipelineId, e);
                }
            }
            ...
            throw new IllegalStateException("Internal tool pipeline '" + toolPipelineId + "' failed", e);
        } finally {
    ...
    public static class KompilePipelineProviderService {
        ...
        public KompilePipelineProviderService(DataFactory dataFactory) {
            logger.info("KompilePipelineProviderService initialized with no built-in pipelines; real pipelines must be registered by configuration.");
        }

        public String getDefaultPipelineId() { return pipelines.containsKey(defaultPipelineId) ? defaultPipelineId : null; }
        ...
        public void registerPipeline(String id, Pipeline pipeline, String purpose, String inputSchemaDescription) {
            if (id == null || id.isBlank()) {
                throw new IllegalArgumentException("Pipeline id is required");
            }
            if (pipeline == null) {
                throw new IllegalArgumentException("Pipeline '" + id + "' is null");
            }
            pipelines.put(id, new PipelineMetadataWrapper(pipeline,
                    purpose == null ? "" : purpose,
                    inputSchemaDescription == null ? "{}" : inputSchemaDescription));
        }

        public List<PipelineToolDefinition> getAvailableToolDefinitions() {
            List<PipelineToolDefinition> toolDefs = new ArrayList<>();
            for (Map.Entry<String, PipelineMetadataWrapper> entry : pipelines.entrySet()) {
                if (!entry.getKey().equals(defaultPipelineId)) {
                    PipelineMetadataWrapper toolMeta = entry.getValue();
                    toolDefs.add(new PipelineToolDefinition(
                            entry.getKey(),
                            toolMeta.getPurpose(),
                            toolMeta.getInputSchemaDescription()
                    ));
                }
            }
            return toolDefs;
