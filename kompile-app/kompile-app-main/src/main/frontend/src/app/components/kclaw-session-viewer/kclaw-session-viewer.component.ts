...
@Component({
  selector: 'app-kclaw-session-viewer',
  standalone: false,
  templateUrl: './kclaw-session-viewer.component.html',
  styleUrls: ['./kclaw-session-viewer.component.css']
})
export class KClawSessionViewerComponent implements OnInit, OnDestroy {
  ...
  loadSessions(): void {
    this.isLoading = true;
    this.kClawService.getSessions().subscribe({
  ...
  selectSession(sessionKey: string): void {
    ...
    this.kClawService.getSessionHistory(sessionKey).subscribe({
      next: (data) => {
  ...
  clearSession(sessionKey: string): void {
    this.kClawService.clearSession(sessionKey).subscribe({
      next: () => {
