 ...
 *
 * Stores per-session, per-Fact visibility settings so users can include or exclude
 * specific sources from a chat session without affecting other sessions or the
 * FactSheet itself.
 *
 * Each row links one ChatSession (by sessionId) to one Fact (by factId) with a
 * SourceMode that controls how the source is used during retrieval.
 *
...
@Table(name = "chat_session_source_contexts", indexes = {
    ...
    @Index(name = "idx_cssc_fact_sheet", columnList = "factSheetId"),
...
public class ChatSessionContext {
    ...
    /**
     * The FactSheet this session is scoped to.
     * Null means global (not scoped to a FactSheet).
     */
    @Column
