...
/**
 * The framework-level beans every kompile server process needs: async execution, an
 * {@link ObjectMapper}, a {@link RestTemplate}, the project store, and the multipart limits.
 *
 * <p>Lives in kompile-app-web-shared rather than app-main because none of it is admin-specific and
 * all three persona processes — the admin console, kompile-app-chat and kompile-app-crawl-manager —
 * break without it. Concretely: {@code ProjectPortabilityJobService} (also web-shared) injects
 * {@code @Qualifier("taskExecutor")}, so the chat app's boot died on
 * {@code NoSuchBeanDefinitionException: java.util.concurrent.Executor} while this class was still
 * app-main-only. The {@code @EnableAsync} here is what makes {@code @Async("taskExecutor")} work in
 * {@code DocumentIngestService} too.
 *
 * <p>{@link VarConfigRegister} registers native-image reflection hints. It is inert outside AOT
 * processing, and every entry is guarded by {@code registerIfPresent}, so an app whose classpath
 * omits a type simply contributes no hint for it — which is why this stays one shared registrar
...
public class AppConfig {
    ...
    public MultipartConfigElement multipartConfigElement() {
        ...
        // Retrieve values from Environment (which includes system properties set by
        // KompileServerRuntime.bootstrap() from the --kompile.multipart.* command-line args)
        String maxFileSizeValue = environment.getProperty(KompileServerRuntime.MAX_FILE_SIZE_PROPERTY, KompileServerRuntime.DEFAULT_MAX_FILE_SIZE);
        String maxRequestSizeValue = environment.getProperty(KompileServerRuntime.MAX_REQUEST_SIZE_PROPERTY, KompileServerRuntime.DEFAULT_MAX_REQUEST_SIZE);
