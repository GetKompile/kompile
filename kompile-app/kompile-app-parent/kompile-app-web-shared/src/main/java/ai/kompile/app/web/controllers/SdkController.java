...
public class SdkController {
    ...
    public ResponseEntity<?> downloadSdk(@RequestBody Map<String, String> request) {
        ...
        String component = request.getOrDefault("component", "runtime");
        String packageRole = request.get("packageRole");
        ...
        if (variant == null || variant.isBlank()) {
            variant = chip; // compatibility with the existing request field
        }
        if (packageRole == null || packageRole.isBlank()) {
            packageRole = platformClassifier.startsWith("android") ? "android-aar"
                    : platformClassifier.startsWith("ios") ? "apple-xcframework" : "platform-sdk";
        }

        try {
            KompileModelManager.ResolvedSdxSdkArtifact resolved = modelManager.resolveAndDownloadSdxSdk(
                    sdkVersion, component, packageRole,
                    platformClassifier, variant, null);
            Path artifactPath = resolved.path();
            SdxSdkManifest.Artifact artifact = resolved.artifact();

            Map<String, Object> result = new LinkedHashMap<>();
            result.put("status", "success");
            result.put("platform", artifact.platform());
            result.put("variant", artifact.variant());
            result.put("classifier", artifact.classifier());
            result.put("component", artifact.component());
            result.put("packageRole", artifact.packageRole());
            result.put("path", artifactPath.toString());
            result.put("fileName", artifact.fileName());
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            LOGGER.error("SDK download failed for platform {}: {}", platformClassifier, e.getMessage());
    ...
    public ResponseEntity<Map<String, Object>> getStatus() {
        ...
        for (SdkDescriptor sdk : modelManager.listAvailableSdks()) {
            for (String classifier : sdk.getPlatformArtifacts().keySet()) {
                totalSdkPlatforms++;
                if (modelManager.isSdkCached(sdk.getSdkId(), sdk.getVersion(), classifier)) {
                    cachedSdks++;
                }
            }
        }
    ...
    public ResponseEntity<?> scaffoldProject(@RequestBody Map<String, Object> request) {
        try {
            ...
            String sdkVersion = (String) request.get("sdkVersion");
            String sdkComponent = (String) request.getOrDefault("sdkComponent", "runtime");
            String sdkPackageRole = (String) request.get("sdkPackageRole");
            String sdkVariant = (String) request.get("sdkVariant");
            String sdkBaseUrl = (String) request.get("sdkBaseUrl");
            ...
            ScaffoldService.ScaffoldRequest scaffoldRequest = new ScaffoldService.ScaffoldRequest(
                    ...
                    sdkVersion, sdkComponent, sdkPackageRole, sdkVariant, sdkBaseUrl
            );
