...
import ai.kompile.app.sync.service.NoteSyncConnectionService.SyncAlreadyRunningException;
...
public class NoteSyncController {
    ...
    public ResponseEntity<SyncRunResponse> triggerSync(@PathVariable Long id) {
        return ResponseEntity.accepted().body(connectionService.triggerSync(id));
    }

    @PostMapping("/connections/{id}/pull")
    public ResponseEntity<SyncRunResponse> pullUpdates(@PathVariable Long id) {
        return ResponseEntity.accepted().body(connectionService.triggerPull(id));
    }

    @GetMapping("/runs/{sessionId}")
    public ResponseEntity<SyncRunResponse> getRun(@PathVariable String sessionId) {
        return ResponseEntity.ok(connectionService.getRun(sessionId));
    }

    @GetMapping("/runs")
    public ResponseEntity<List<SyncRunResponse>> listRuns(
            @RequestParam(required = false) Long connectionId,
            @RequestParam(required = false) Long factSheetId) {
        return ResponseEntity.ok(connectionService.listRuns(connectionId, factSheetId));
    }
    ...
    public ResponseEntity<Map<String, Object>> handleSyncAlreadyRunning(
            SyncAlreadyRunningException e) {
        Map<String, Object> body = new LinkedHashMap<>();
        body.put("error", e.getMessage());
        if (e.getActiveRunId() != null) {
            body.put("activeRunId", e.getActiveRunId());
        }
        if (e.getLeaseExpiresAt() != null) {
            body.put("leaseExpiresAt", e.getLeaseExpiresAt());
        }
