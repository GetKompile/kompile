...
@Component({
  selector: 'app-kclaw-permission-manager',
  standalone: false,
  templateUrl: './kclaw-permission-manager.component.html',
  styleUrls: ['./kclaw-permission-manager.component.css']
})
export class KClawPermissionManagerComponent implements OnInit, OnDestroy {
  ...
  loadPermissions(): void {
    this.kClawService.getPermissions().subscribe({
      next: (permissions) => {
  ...
  allowCommand(command: string): void {
    this.kClawService.allowCommand(command).subscribe({
      next: () => {
  ...
  denyCommand(command: string): void {
    this.kClawService.denyCommand(command).subscribe({
      next: () => {
