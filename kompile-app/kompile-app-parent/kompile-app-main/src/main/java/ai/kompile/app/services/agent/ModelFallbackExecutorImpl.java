...
public class ModelFallbackExecutorImpl implements LlmFallbackExecutor {
        ...
        while (totalAttempts < config.maxAttempts) {
            ...
            try {
                CompletableFuture<String> future = CompletableFuture.supplyAsync(
                        () -> llmChat.prompt().user(prompt).call().content(),
                        executor
                );
    ...
    List<FallbackEntry> buildDynamicChain(ModelFallbackConfig config, String backendId) {
        ...
        try {
            if (modelService != null && agentRegistry != null) {
                for (AgentProvider agent : agentRegistry.getAllAgents()) {
                    String name = agent.getName();
                    if (name == null || config.paidAgents.contains(name)) {
                        continue; // paid agents are the last-resort tail, added below
                    }
                    for (String modelId : modelService.selectExtractionModels(name)) {
                        chain.add(new FallbackEntry(name, modelId));
                    }
                }
            }
        } catch (Exception e) {
        ...
        if (freeCount == 0) {
            ...
            // chain — which still tries the free agent's default model BEFORE the paid tail, so we
            // never go claude-first. (Returning only the paid tail here would put claude first,
            // violating the never-claude-for-extraction mandate.)
        ...
        log.info("[ModelFallback][{}] dynamic chain: {} free model(s) then {} paid last-resort entr(ies)",
                backendId, freeCount, chain.size() - freeCount);
        return chain;
    }
