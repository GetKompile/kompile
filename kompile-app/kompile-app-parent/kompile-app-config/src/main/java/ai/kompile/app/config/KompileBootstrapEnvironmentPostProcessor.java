...
public class KompileBootstrapEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        ...
        // --- Per-project data root, resolved IN THE LIBRARY (no -D launch flag required) ---
        // Honour an explicit kompile.data.dir (-D or --arg); otherwise resolve the project root by
        // walking up the working directory for a kompile.project.json (a generated project), else the
        // global ~/.kompile — KompileHome.resolvedProjectDirectory() is the shared implementation.
        // Republish as a system property so static KompileHome callers (config dirs, model cache) AND
        // Spring @Value("${kompile.data.dir}") services all resolve the same root.
        String dataDir = environment.getProperty("kompile.data.dir");
        if (dataDir == null || dataDir.isBlank()) {
            dataDir = KompileHome.resolvedProjectDirectory().getAbsolutePath();
        }
        System.setProperty("kompile.data.dir", dataDir);
        defaults.put("kompile.data.dir", dataDir);
        ...
        // --- LLM provider keys: bridge the canonical env vars the UI/diagnostics document
        // (OPENAI_API_KEY / ANTHROPIC_API_KEY) to the Spring AI properties. Relaxed binding
        // only maps SPRING_AI_* names, so without this an installed app (no
        // application.properties) never sees the documented vars. Gemini is Vertex-based
        // (project-id credentials), so it has no api-key property to bridge.
        bridgeEnvKey(environment, defaults, "OPENAI_API_KEY", "spring.ai.openai.api-key");
        bridgeEnvKey(environment, defaults, "ANTHROPIC_API_KEY", "spring.ai.anthropic.api-key");

        environment.getPropertySources().addLast(new MapPropertySource(PROPERTY_SOURCE_NAME, defaults));
    }

    private static void bridgeEnvKey(ConfigurableEnvironment environment, Map<String, Object> defaults,
                                     String envVar, String property) {
        String value = System.getenv(envVar);
        if (value != null && !value.isBlank() && environment.getProperty(property) == null) {
