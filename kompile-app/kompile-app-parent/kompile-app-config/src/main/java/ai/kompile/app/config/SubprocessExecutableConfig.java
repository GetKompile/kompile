    ...
    private String resolveNativePath() {
        ...
        if (NativeImageInfo.isRunningInNativeImage()) {
            ...
            if (execPath != null) {
                ...
                logger.warn("Detected native executable path is not an executable file: {}", execPath);
            }
        }
        ...
        for (String searchPath : searchPaths) {
            try {
                ...
                // isRegularFile is load-bearing: Files.isExecutable() is true for
                // traversable DIRECTORIES too — "./kompile-app" from the repo root
                // matched the kompile-app/ module dir and exec failed with EACCES.
                if (Files.isRegularFile(path) && Files.isExecutable(path)) {
