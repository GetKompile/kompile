...
public class UnifiedCrawlController {
    ...
    public ResponseEntity<?> getJob(@PathVariable String jobId) {
        ...
        return unifiedCrawlService.getJob(resolvedId)
                .map(job -> {
                    ...
                    // Entity-partition coverage. Same rule as the routing stats above: the counters
                    // are in the snapshot, and a subject with no coverage claim is exactly what a
                    // reader has to know before trusting an answer from this graph — so it goes in
                    // the map, always, not only when something went wrong.
                    result.put("partitionsCovered", snapshot.getPartitionsCovered());
                    result.put("partitionsUncovered", snapshot.getPartitionsUncovered());
                    result.put("uncoveredPartitionSubjects",
                            snapshot.getUncoveredPartitionSubjects() != null
                                    ? snapshot.getUncoveredPartitionSubjects() : List.of());
                    if (snapshot.getPartitionCoverageDetail() != null) {
                        result.put("partitionCoverageDetail", snapshot.getPartitionCoverageDetail());
                    }
    ...
    private Map<String, Object> buildJobDetailMap(UnifiedCrawlJob job, UnifiedCrawlJob.ProgressSnapshot snap) {
        ...
        // Entity-partition coverage travels into the persisted detail too: which subjects a
        // finished crawl left without a coverage claim is exactly the thing worth looking back at,
        // and the in-memory job it came from is gone by then.
        m.put("partitionsCovered", snap.getPartitionsCovered());
        m.put("partitionsUncovered", snap.getPartitionsUncovered());
        m.put("uncoveredPartitionSubjects", snap.getUncoveredPartitionSubjects() != null
                ? snap.getUncoveredPartitionSubjects() : List.of());
        if (snap.getPartitionCoverageDetail() != null) {
            m.put("partitionCoverageDetail", snap.getPartitionCoverageDetail());
        }
    ...
    private IngestPhase mapCrawlPhaseToIngestPhase(UnifiedCrawlJob.ProgressSnapshot snap) {
        ...
        return switch (phase) {
            ...
            // Graph work that runs after indexing. IngestPhase has no post-index bucket, so these
            // report as graph extraction — the alternative is the default below, which would show a
            // crawl falling back to LOADING once it is nearly finished.
            case "ENTITY_PARTITIONS", "ENRICHMENT" -> IngestPhase.GRAPH_EXTRACTION;
