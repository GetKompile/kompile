    ...
    public ExtractionResult extract(Document doc) {
        ...
        if (bodyText != null && !bodyText.isBlank()) {
            ...
            for (String mentioned : mentionedUsers) {
                // An accountId-reference mention keys on the raw accountId — matching the author /
                // modifier / comment-author entities (which key on accountId) — so the same user
                // resolves to ONE person node instead of splitting a display-name mention from an
                // accountId-keyed author. Display-name-only mentions still key on the lowercase name
                // (no accountId available; resolution can later merge them by name).
                boolean isAccountId = mentioned.startsWith("accountid:");
                String accountId = isAccountId ? mentioned.substring(10) : null;
                String displayName = isAccountId ? accountId : mentioned;
                String mentionKey = accountId != null ? accountId : displayName.toLowerCase();
                String mentionEntityId = entityId("person", mentionKey);
                Map<String, String> mentionProps = new LinkedHashMap<>();
                mentionProps.put("displayName", displayName);
                if (accountId != null) mentionProps.put("accountId", accountId);
