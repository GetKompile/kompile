...
public class GraphSubprocessMain {
    ...
    private static GraphSubprocessArgs.GraphExtractionResult runWithGraphConstructor(
        ...
        for (int batchIdx = 0; batchIdx < totalBatches; batchIdx++) {
            ...
            try {
                ...
                logger.error("Batch {}/{} failed: {}", batchIdx + 1, totalBatches, e.getMessage(), e);
                throw new IllegalStateException("Graph subprocess extraction batch "
                        + (batchIdx + 1) + "/" + totalBatches + " failed: " + e.getMessage(), e);
            }
        }
    ...
    private static void requireNonEmptyExtractionGraph(List<RetrievedDoc> docs, Graph graph) {
        ...
        if (entities + relationships == 0) {
            throw new IllegalStateException("GraphConstructor produced empty extraction graph for "
                    + docs.size() + " non-empty document chunk(s)");
        }
    }
    ...
    private static String truncateStackTrace(Exception e) {
        return ThrowableUtils.stackTraceString(e, 10);
    }
}
