...
public class VectorPopulationSubprocessMain {
    ...
    private static EmbeddingModel selectEmbeddingModel(AnnotationConfigApplicationContext context) {
        ...
        if (selected == null) {
            throw new IllegalStateException("No real embedding model found in subprocess context; refusing to populate vectors with NoOp embeddings");
        }
    ...
    private static IndexerService selectIndexerService(AnnotationConfigApplicationContext context) {
        ...
        if (selected == null) {
            throw new IllegalStateException("No real indexer service found in vector population subprocess context");
        }
    ...
    private static void logSubprocessFeatures(AnnotationConfigApplicationContext context,
        ...
        try {
            ...
            if (fullyFunctional) {
                ...
                if (!hasRealEmbedding) logger.info("│   - No real embedding model selected - vector population cannot continue │");
                if (!hasRealIndexer) logger.info("│   - No real indexer service selected - vector population cannot continue │");
            }
        } catch (Exception e) {
