    ...
    public PipelineResult execute(SubprocessArgs args, SubprocessProgressReporter reporter,
        ...
        try {
            ...
            if (documents.isEmpty()) {
                throw new PipelineException(phase, "Loader produced zero documents for file: " + filePath);
            }
            ...
            try (ParallelIngestPipeline pipeline = new ParallelIngestPipeline(
                ...
                if (pipelineResult.chunksCreated() <= 0) {
                    throw new PipelineException("CHUNKING", "Pipeline created zero chunks for file: " + filePath);
                }
                if (pipelineResult.chunksIndexed() <= 0) {
                    throw new PipelineException("INDEXING", "Pipeline indexed zero chunks for file: " + filePath);
                }
