...
public class EmbeddingStage implements PipelineStage<ChunkingStage.ChunkingOutput, EmbeddingStage.EmbeddingOutput> {
        ...
        try {
            ...
            while (i < totalChunks) {
                ...
                for (RetrievedDoc chunk : batch) {
                    ...
                    batchBytes += (text != null ? text.length() : 0) * 2L;
                }

                long batchStartNanos = System.nanoTime();
                long batchTimeMs = 0;
                INDArray batchEmbeddings = null;
                try {
                    batchEmbeddings = embeddingModel.embed(texts);
                    batchTimeMs = (System.nanoTime() - batchStartNanos) / 1_000_000;
                    validateBatchEmbeddings(batchEmbeddings, batch.size());

                    // getRow() returns a view over batchEmbeddings; dup() gives each output row
                    // independent native storage before the batch array is closed.
                    for (int j = 0; j < batch.size(); j++) {
                        INDArray embedding = batchEmbeddings.getRow(j).dup();
                        validateEmbedding(embedding, "chunk " + batch.get(j).getId());
                    ...
                    batchFailureCount++;
                    int oldSize = effectiveBatchSize;
                    effectiveBatchSize = reduceBatchSizeAfterFailure(effectiveBatchSize);
                    if (batchFailureCount <= MAX_BATCH_RETRIES) {
                        long backoffMs = retryBackoffMs(batchFailureCount);
                        logger.warn("Embedding batch {}-{} failed (attempt {}/{}): {}. Retrying with batch size {} after {}ms",
                                i, batchEnd, batchFailureCount, MAX_BATCH_RETRIES, e.getMessage(),
                                effectiveBatchSize, backoffMs);
                        if (effectiveBatchSize == oldSize && batch.size() > effectiveBatchSize) {
                            logger.debug("Retrying unchanged effective size because current batch was already bounded by remaining work");
                        }
                        sleepBeforeRetry(backoffMs);
                        continue;
                    }
                    throw new IllegalStateException("Embedding batch " + i + "-" + batchEnd
                            + " failed after " + batchFailureCount + " attempt(s): " + e.getMessage(), e);
                } finally {
                    closeQuietly(batchEmbeddings, "batch embeddings");
                }
            }
    ...
    private int reduceBatchSizeAfterFailure(int currentSize) {
        return Math.max(MIN_BATCH_SIZE, Math.min(currentSize, currentSize / 2));
    }
    ...
    private void validateBatchEmbeddings(INDArray batchEmbeddings, int expectedRows) {
        if (batchEmbeddings == null) {
            throw new IllegalStateException("Embedding model returned null batch embeddings");
        }
        if (batchEmbeddings.rank() < 2) {
            throw new IllegalStateException("Embedding model returned rank " + batchEmbeddings.rank()
                    + " batch embeddings; expected rank >= 2");
        }
        if (batchEmbeddings.size(0) < expectedRows) {
            throw new IllegalStateException("Embedding model returned " + batchEmbeddings.size(0)
                    + " row(s) for " + expectedRows + " chunk(s)");
        }
    }

    private void validateEmbedding(INDArray embedding, String label) {
        if (embedding == null || embedding.length() == 0) {
            throw new IllegalStateException("Embedding for " + label + " is null or empty");
        }
        float[] values = embedding.data().asFloat();
        double magnitude = 0.0;
        for (float value : values) {
            if (!Float.isFinite(value)) {
                throw new IllegalStateException("Embedding for " + label + " contains non-finite values");
            }
            magnitude += (double) value * value;
        }
        if (!Double.isFinite(magnitude) || magnitude <= MIN_EMBEDDING_MAGNITUDE) {
            throw new IllegalStateException("Embedding for " + label + " has zero or invalid magnitude");
        }
    }

    private void closeQuietly(INDArray array, String label) {
        if (array == null) {
            return;
        }
        try {
            ...
            logger.trace("Error closing {}: {}", label, e.getMessage());
        }
    }
