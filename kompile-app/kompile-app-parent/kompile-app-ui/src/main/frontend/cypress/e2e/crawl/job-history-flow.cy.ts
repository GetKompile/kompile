...
describe('Job History Flows', () => {
  ...
  // Subprocess events are an admin surface — model launches, staging, native workers — so they
  // are asked of the admin console, not the crawl manager the rest of this spec drives. A job
  // that failed because its subprocess died shows the failure here and the cause there, which is
  // exactly why the crawl UI links across to admin rather than mounting the endpoint itself.
  describe('Subprocess Events', () => {
    it('should return recent subprocess events', () => {
      cy.apiGetFrom('admin', '/subprocess-events/recent?hours=168').then((res) => {
        expect(res.status).to.eq(200);
        expect(res.body).to.be.an('array');
