...
describe('Knowledge Base Selector', () => {
  ...
  // The routes below are chat-app's own — chat, project, knowledge, fact-sheets, graph. `/#/tools`
  // and `/#/settings` are admin routes and no longer resolve here; visiting them would land on the
  // wildcard redirect and quietly assert the selector on whatever route that lands on.
  describe('KB Selector Across Routes', () => {
    const CHAT_ROUTES = ['/#/chat', '/#/project', '/#/knowledge', '/#/fact-sheets', '/#/graph'];

    it('should be visible on every route', () => {
      CHAT_ROUTES.forEach(route => {
        cy.visit(route);
