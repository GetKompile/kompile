...
import {
  ...
  Workflow,
  WorkflowStep
} from '@shared/services/orchestrator.service';
import { AgentService } from '@shared/services/agent.service';
import { FactSheetService } from '@shared/services/fact-sheet.service';
import { AgentProvider, FactSheet } from '@shared/models/api-models';
import { TaskDefinition } from '@shared/models/orchestrator-models';
...
describe('OrchestratorHubComponent', () => {
  ...
  beforeEach(async () => {
    orchestratorServiceSpy = jasmine.createSpyObj('OrchestratorService', [
      ...
      /*
       * The hub's later tabs — enhanced task definitions, state machine, LLM sessions and the
       * agent/subprocess log viewers — call these. A jasmine spy object only has the methods
       * named here, so every one omitted is an "is not a function" the moment its tab loads.
       */
      'getEnhancedTaskDefinitions', 'deleteEnhancedTaskDefinition', 'testTaskDefinition',
      'executeTask', 'getStates', 'transitionTo', 'getContext', 'updateContext',
      'getActiveLlmSessions', 'getLlmSessionHistory', 'getSessionConversation',
      'cancelLlmSession', 'invokeLlm', 'getLlmProviders', 'getLlmTriggers', 'registerLlmTrigger',
      'getAgentLogRuns', 'getAgentLogRecords', 'getSubprocessLogRuns', 'getSubprocessLogRecords'
    ]);

    // Defaults for the above. Every one is subscribed by the component, so a spy left without a
    // return value trades "is not a function" for "cannot read 'subscribe' of undefined".
    orchestratorServiceSpy.getEnhancedTaskDefinitions.and.returnValue(of([]));
    orchestratorServiceSpy.deleteEnhancedTaskDefinition.and.returnValue(of({} as any));
    orchestratorServiceSpy.testTaskDefinition.and.returnValue(of({} as any));
    orchestratorServiceSpy.executeTask.and.returnValue(of(mockTask));
    orchestratorServiceSpy.getStates.and.returnValue(of([]));
    orchestratorServiceSpy.transitionTo.and.returnValue(of({} as any));
    orchestratorServiceSpy.getContext.and.returnValue(of({} as any));
    orchestratorServiceSpy.updateContext.and.returnValue(of({} as any));
    orchestratorServiceSpy.getActiveLlmSessions.and.returnValue(of([]));
    orchestratorServiceSpy.getLlmSessionHistory.and.returnValue(of([]));
    orchestratorServiceSpy.getSessionConversation.and.returnValue(of([] as any));
    orchestratorServiceSpy.cancelLlmSession.and.returnValue(of({} as any));
    orchestratorServiceSpy.invokeLlm.and.returnValue(of({} as any));
    orchestratorServiceSpy.getLlmProviders.and.returnValue(of([]));
    orchestratorServiceSpy.getLlmTriggers.and.returnValue(of([]));
    orchestratorServiceSpy.registerLlmTrigger.and.returnValue(of({} as any));
    orchestratorServiceSpy.getAgentLogRuns.and.returnValue(of([]));
    orchestratorServiceSpy.getAgentLogRecords.and.returnValue(of([]));
    orchestratorServiceSpy.getSubprocessLogRuns.and.returnValue(of([]));
    orchestratorServiceSpy.getSubprocessLogRecords.and.returnValue(of([]));
  ...
  describe('editTaskDefinition', () => {
    it('should select the definition and swap the list view for the editor', () => {
      ...
      // No tab switch to assert: the Edit button that calls this only renders inside the
      // definitions tab, so the user is already there. showDefinitionEditor is the actual effect —
      // it swaps the definition list out for the editor within the same panel.
