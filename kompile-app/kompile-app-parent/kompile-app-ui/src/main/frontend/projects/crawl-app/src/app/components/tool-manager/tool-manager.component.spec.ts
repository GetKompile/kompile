...
import { ToolDefinitionService } from '@shared/services/tool-definition.service';
import { EnhancedToolDefinition, ToolsSummary } from '@shared/models/api-models';
...
describe('ToolManagerComponent', () => {
  ...
  // navigator.clipboard is a process-wide global and karma runs every spec file in one browser
  // context, so replacing it without restoring leaks a jasmine spy into every file that runs later.
  // The next spec to call spyOn(navigator.clipboard, 'writeText') then dies with "writeText has
  // already been spied upon" — a failure in a file that never touched the clipboard. Swap and
  // restore around each test so the contamination cannot escape this describe.
  describe('copyAgentPrompt()', () => {
    ...
    beforeEach(() => {
      ...
      mockClipboard = { writeText: jasmine.createSpy('writeText').and.returnValue(Promise.resolve()) };
    ...
    afterEach(() => {
      if (originalClipboard) {
        ...
        // Came from Navigator.prototype — dropping the own override hands it back.
