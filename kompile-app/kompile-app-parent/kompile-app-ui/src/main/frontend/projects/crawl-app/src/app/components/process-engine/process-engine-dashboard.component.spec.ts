...
import { ProcessEngineService, WorkflowRun, ApprovalRequest } from '@shared/services/process-engine.service';
...
describe('ProcessEngineDashboardComponent', () => {
  ...
  beforeEach(async () => {
    mockService = jasmine.createSpyObj('ProcessEngineService', [
      ...
      // Loaded during init alongside the run lists; a spy object has only the methods named
      // here, so leaving it out makes the component's call throw "is not a function".
      'listStoredSuggestions'
    ]);
    mockService.listStoredSuggestions.and.returnValue(of({ count: 0, suggestions: [] }));
