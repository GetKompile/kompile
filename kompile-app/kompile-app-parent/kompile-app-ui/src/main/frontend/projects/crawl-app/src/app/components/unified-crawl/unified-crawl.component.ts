...
import {
  ...
  ResumableJobEntry
} from '@shared/services/unified-crawl.service';
...
import { JobLogService, JobLogEntry } from '@shared/services/job-log.service';
import { FactSheetService } from '@shared/services/fact-sheet.service';
import { FactSheet } from '@shared/models/api-models';
import { GraphExtractionService, ModelProvider, GraphExtractionConfig } from '@shared/services/graph-extraction.service';
import { WebSocketService } from '@shared/services/websocket.service';
import { DistributedCrawlService } from '@shared/services/distributed-crawl.service';
import { NoteSyncService } from '@shared/services/note-sync.service';
import {
  ProjectService,
...
export class UnifiedCrawlComponent implements OnInit, OnDestroy {
  ...
  syncRuns = new Map<number, SyncRunResponse>();
  ...
  restorationReadiness: RestorationReadiness | null = null;
  restorationResult: RestorationResult | null = null;
  restorationActionInProgress = false;
  restorationInspectionJobId: string | null = null;
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(
      this.wsService.subscribeToSyncProgress().subscribe((update: SyncStatusUpdate) => {
        ...
        if (this.isTerminalSyncStatus(update.status)) {
        ...
        if (update.sessionId) this.loadSyncRun(update.sessionId);
        this.cdr.markForCheck();
      })
    );
    this.pollInterval = setInterval(() => {
      ...
      this.refreshResumableJobs();
      this.refreshPortableJobs();
      const durableRunNeedsRefresh = Array.from(this.syncRuns.values()).some(run =>
        run.status === 'QUEUED'
        || run.status === 'RUNNING'
        || run.graphStatus === 'PENDING');
      if (this.pullingConnections.size > 0
          || durableRunNeedsRefresh
          || this.syncConnections.some(connection => !!connection.activeSyncRunId)) {
  ...
  formatPhase(phase: string | undefined | null): string {
    ...
    const phaseMap: { [key: string]: string } = {
      ...
      'ENTITY_PARTITIONS': 'Entity partition coverage',
  ...
  loadRestorationReadiness(): void {
    this.restorationActionInProgress = true;
    this.restorationInspectionJobId = null;
    this.subscriptions.add(this.projectService.getRestorationReadiness().subscribe({
      next: readiness => {
        this.restorationReadiness = readiness;
        this.restorationActionInProgress = false;
        this.cdr.markForCheck();
      },
      error: err => {
        this.restorationActionInProgress = false;
        this.portabilityError = err.error?.error || 'Failed to load restoration readiness';
        this.cdr.markForCheck();
      }
    }));
  }

  inspectImportedRestoration(job: PortableKnowledgeBaseJob): void {
    if (job.operation !== 'IMPORT' || job.status !== 'COMPLETED') return;
    this.restorationActionInProgress = true;
    this.portabilityError = null;
    this.subscriptions.add(this.projectService.getImportedRestorationReadiness(job.id).subscribe({
      next: readiness => {
        this.restorationReadiness = readiness;
        this.restorationInspectionJobId = job.id;
      ...
      error: err => {
        this.restorationActionInProgress = false;
        this.portabilityError = err.error?.error || 'Failed to inspect imported project readiness';
        this.cdr.markForCheck();
      }
    }));
  }

  restorePortableCatalogs(dryRun: boolean): void {
    this.restorationActionInProgress = true;
    this.portabilityError = null;
    this.subscriptions.add(this.projectService.restorePortableCatalogs(dryRun).subscribe({
      next: result => {
        this.restorationResult = result;
        this.restorationReadiness = result.readiness;
        this.restorationActionInProgress = false;
        const message = dryRun
          ? `Plan: ${result.plannedFactSheets} fact sheets and ${result.plannedSourceConnections} sources`
          : `Restored ${result.restoredFactSheets} fact sheets and ${result.restoredSourceConnections} sources`;
        this.snackBar.open(message, 'OK', { duration: 4000 });
        if (!dryRun && this.activeFactSheet?.id) this.loadSyncConnections(this.activeFactSheet.id);
        this.cdr.markForCheck();
      },
      error: err => {
        this.restorationActionInProgress = false;
        this.portabilityError = err.error?.error || 'Failed to restore portable catalogs';
        this.cdr.markForCheck();
      }
    }));
  }
  ...
  pullSourceUpdates(connection: SyncConnectionResponse): void {
    ...
    this.syncProgress.set(connection.id, {
      ...
      deleted: 0, conflicts: 0, skipped: 0, errors: 0, timestamp: new Date().toISOString()
    });
    this.cdr.markForCheck();
    this.subscriptions.add(this.noteSyncService.pullUpdates(connection.id).subscribe({
      next: result => {
        this.syncRuns.set(connection.id, result);
        this.syncProgress.set(connection.id, this.syncRunAsProgress(result));
        this.snackBar.open(`Pull queued (${result.sessionId})`, 'OK', { duration: 3000 });
        this.cdr.markForCheck();
      },
      error: err => {
        this.pullingConnections.delete(connection.id);
        if (err.error?.activeRunId) this.loadSyncRun(err.error.activeRunId);
  ...
  bindSourceCredential(connection: SyncConnectionResponse): void {
    const secret = (this.sourceCredentialDrafts[connection.id] || '').trim();
    if (!secret || !this.requiresSourceSecret(connection)) return;
    this.sourceAuthUpdating.add(connection.id);
    const request: SyncConnectionRequest = {
      factSheetId: connection.factSheetId,
      provider: connection.provider,
      externalScope: connection.externalScope,
      direction: connection.direction,
      pollCron: connection.pollCron,
      obsidianApiUrl: connection.obsidianApiUrl,
      repositoryUrl: connection.repositoryUrl,
      gitBranch: connection.gitBranch,
      gitUsername: connection.gitUsername,
      authMode: connection.authMode,
      autoCommit: connection.autoCommit,
      remoteSyncEnabled: connection.remoteSyncEnabled
    };
    if (connection.authMode === 'OBSIDIAN_REST_TOKEN') request.obsidianToken = secret;
    if (connection.authMode === 'HTTPS_TOKEN') request.gitToken = secret;
    this.subscriptions.add(this.noteSyncService.updateConnection(connection.id, request).subscribe({
      next: updated => {
        this.sourceCredentialDrafts[connection.id] = '';
        this.replaceSyncConnection(updated);
        this.sourceAuthUpdating.delete(connection.id);
        this.snackBar.open('Credential bound. Run Test Auth before enabling.', 'OK', { duration: 3500 });
        this.loadRestorationReadiness();
        this.cdr.markForCheck();
      },
      error: err => {
        this.sourceAuthUpdating.delete(connection.id);
        this.snackBar.open(err.error?.error || 'Failed to bind source credential', 'Dismiss', { duration: 4000 });
        this.cdr.markForCheck();
      }
    }));
  }

  testSourceAuth(connection: SyncConnectionResponse): void {
    this.sourceAuthUpdating.add(connection.id);
    this.subscriptions.add(this.noteSyncService.testConnectionAuth(connection.id).subscribe({
      next: result => {
        this.sourceAuthUpdating.delete(connection.id);
      ...
      error: err => {
        this.sourceAuthUpdating.delete(connection.id);
        this.snackBar.open(err.error?.error || 'Source authentication test failed', 'Dismiss', { duration: 4000 });
        this.cdr.markForCheck();
      }
    }));
  }

  toggleSourceEnabled(connection: SyncConnectionResponse): void {
    this.sourceAuthUpdating.add(connection.id);
    const operation = connection.enabled
      ? this.noteSyncService.disableConnection(connection.id)
      : this.noteSyncService.enableConnection(connection.id);
    this.subscriptions.add(operation.subscribe({
      next: updated => {
        this.replaceSyncConnection(updated);
        this.sourceAuthUpdating.delete(connection.id);
        this.snackBar.open(updated.enabled ? 'Source enabled' : 'Source disabled', 'OK', { duration: 2500 });
        this.loadRestorationReadiness();
        this.cdr.markForCheck();
      },
      error: err => {
        this.sourceAuthUpdating.delete(connection.id);
        this.snackBar.open(err.error?.error || 'Failed to change source state', 'Dismiss', { duration: 4000 });
        this.cdr.markForCheck();
      }
    }));
  }

  requiresSourceSecret(connection: SyncConnectionResponse): boolean {
    return connection.authMode === 'OBSIDIAN_REST_TOKEN' || connection.authMode === 'HTTPS_TOKEN';
  }

  sourceAuthReady(connection: SyncConnectionResponse): boolean {
    const authRequired = connection.provider === 'NOTION'
      || (!!connection.authMode && connection.authMode !== 'NONE');
    return !authRequired || connection.authStatus === 'VALID';
  }

  canOperateSource(connection: SyncConnectionResponse): boolean {
    return connection.enabled && this.sourceAuthReady(connection);
  }

  private replaceSyncConnection(updated: SyncConnectionResponse): void {
    this.syncConnections = this.syncConnections.map(item =>
      item.id === updated.id ? updated : item);
  }

  isPulling(connectionId: number): boolean {
    const status = this.syncRuns.get(connectionId)?.status || this.syncProgress.get(connectionId)?.status;
    return this.pullingConnections.has(connectionId) || status === 'QUEUED' || status === 'RUNNING';
  }

  loadRecentSyncRuns(): void {
    this.subscriptions.add(this.noteSyncService.listRuns(
      undefined, this.activeFactSheet?.id).subscribe({
      next: runs => {
        const connectionIds = new Set(this.syncConnections.map(connection => connection.id));
        const latest = new Map<number, SyncRunResponse>();
        for (const run of runs) {
          if (connectionIds.has(run.connectionId) && !latest.has(run.connectionId)) {
            latest.set(run.connectionId, run);
          }
        }
        this.syncRuns = latest;
        for (const run of latest.values()) {
          this.syncProgress.set(run.connectionId, this.syncRunAsProgress(run));
          if (run.status === 'QUEUED' || run.status === 'RUNNING') {
            this.pullingConnections.add(run.connectionId);
          } else {
            this.pullingConnections.delete(run.connectionId);
          }
        }
        this.cdr.markForCheck();
      },
      error: err => console.error('Failed to load durable source sync runs:', err.message)
    }));
  }

  loadSyncRun(sessionId: string): void {
    this.subscriptions.add(this.noteSyncService.getRun(sessionId).subscribe({
      next: run => {
        this.syncRuns.set(run.connectionId, run);
        this.syncProgress.set(run.connectionId, this.syncRunAsProgress(run));
        if (this.isTerminalSyncStatus(run.status)) {
          this.pullingConnections.delete(run.connectionId);
        }
        this.cdr.markForCheck();
      },
      error: () => { /* A legacy WebSocket session may not have a durable run. */ }
    }));
  }

  openSourceGraphJob(run: SyncRunResponse): void {
    if (run.crawlJobId) this.selectJob(run.crawlJobId);
  }

  private syncRunAsProgress(run: SyncRunResponse): SyncStatusUpdate {
    return {
      ...
      skipped: run.skipped,
      errors: run.errors,
      crawlJobId: run.crawlJobId,
      graphStatus: run.graphStatus,
      timestamp: run.updatedAt
    };
  }

  private isTerminalSyncStatus(status: string): boolean {
