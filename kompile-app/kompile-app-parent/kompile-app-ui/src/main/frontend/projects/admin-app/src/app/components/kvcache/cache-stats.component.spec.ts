...
import { KVCacheService } from '@shared/services/kvcache.service';
import { KVCacheStats, KVCacheSummary, StatsSample } from '@shared/models/kvcache-models';
...
describe('CacheStatsComponent', () => {
  ...
  it('should start auto-refresh interval when autoRefresh is true', fakeAsync(() => {
    ...
    // toggleAutoRefresh() polls on interval(10000); ticking any less never reaches the first
    // emission, so a shorter tick fails this assertion whether or not the interval was started.
    tick(10000);
