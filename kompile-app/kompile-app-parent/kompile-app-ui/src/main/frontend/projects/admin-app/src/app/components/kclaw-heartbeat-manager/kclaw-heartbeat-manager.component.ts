...
import { HeartbeatInfo, AgentDefinition } from '@shared/models/kclaw-models';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-kclaw-heartbeat-manager',
  standalone: false,
  templateUrl: './kclaw-heartbeat-manager.component.html',
  styleUrls: ['./kclaw-heartbeat-manager.component.css']
})
export class KClawHeartbeatManagerComponent implements OnInit, OnDestroy {
  ...
  createHeartbeat(): void {
    ...
    this.kClawService.createHeartbeat(this.heartbeatForm.value).subscribe({
      next: () => {
  ...
  cancelHeartbeat(id: string): void {
    this.kClawService.cancelHeartbeat(id).subscribe({
      next: () => {
