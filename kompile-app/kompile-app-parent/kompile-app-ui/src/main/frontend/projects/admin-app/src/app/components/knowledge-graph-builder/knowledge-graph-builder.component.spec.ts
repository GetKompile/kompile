...
import { KnowledgeGraphBuilderService } from '@shared/services/knowledge-graph-builder.service';
import {
  GraphBuilderInfo,
  ExtractionJob,
  TripleProposal,
  ...
  BUILDER_TYPE_ICONS
} from '@shared/models/graph-builder-models';
...
describe('KnowledgeGraphBuilderComponent', () => {
  ...
  beforeEach(async () => {
    serviceSpy = jasmine.createSpyObj<KnowledgeGraphBuilderService>('KnowledgeGraphBuilderService', [
      ...
      // Backs the per-job statistics panel. A spy object only has the methods named here, so
      // omitting one turns a component call into "is not a function" at runtime.
      'getJobStatistics'
    ]);
    ...
    serviceSpy.getJobStatistics.and.returnValue(of({} as any));
