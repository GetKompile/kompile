...
import {
  ...
  IngestPhase,
  FailureReason
} from '@shared/services/job-history.service';
import { WebSocketService, WebSocketConnectionState } from '@shared/services/websocket.service';
import { UnifiedCrawlService } from '@shared/services/unified-crawl.service';
import { IngestStatus } from '@shared/models/api-models';
...
describe('JobHistoryComponent', () => {
  ...
  let unifiedCrawlSpy: jasmine.SpyObj<UnifiedCrawlService>;
  ...
  beforeEach(async () => {
    ...
    /*
     * The component injects UnifiedCrawlService for its per-step panel (getJobFromHistory,
     * runStep). Left unstubbed, the real one is constructed from the root injector and asks
     * for HttpClient, which this TestBed does not provide — so every spec in this file died
     * in beforeEach with a NullInjectorError, not just the step-panel ones.
     */
    unifiedCrawlSpy = jasmine.createSpyObj<UnifiedCrawlService>('UnifiedCrawlService', [
      'getJobFromHistory',
      'runStep'
    ]);
    unifiedCrawlSpy.getJobFromHistory.and.returnValue(of({ steps: [] } as any));
    unifiedCrawlSpy.runStep.and.returnValue(of({} as any));
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: UnifiedCrawlService, useValue: unifiedCrawlSpy }
      ],
