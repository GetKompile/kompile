...
import { GraphFolRulesService, RuleDto } from '@shared/services/graph-fol-rules.service';
...
describe('FolRulesBrowserComponent', () => {
  ...
  beforeEach(async () => {
    ...
    // MatSnackBar has to be swapped with overrideProvider rather than a `providers` entry. The
    // component is standalone and imports MatSnackBarModule, and MatSnackBar is declared
    // `providedIn: MatSnackBarModule`, so that provider lands in the same environment injector and
    // wins over the testing module's. A `providers` entry still compiles and reads correctly, but
    // leaves the component holding the real snack bar — every assertion on the spy then reports
    // "never called" with no error to explain it.
  ...
  it('weightBarPct scales against 1.0 while every rule sits at or below it', () => {
    ...
    // maxWeight floors at 1.0, so the bar reads against the PSL reference weight rather than
    // against whichever rule happens to be strongest on screen. A set of uniformly weak rules
    // renders weak instead of having its best member stretched into a full bar that overstates it.
    expect(component.weightBarPct(component.allRules[0])).toBeCloseTo(0.4);
    expect(component.weightBarPct(component.allRules[1])).toBeCloseTo(0.8);
  });

  it('weightBarPct rescales against the strongest rule once one exceeds 1.0', () => {
    rulesSpy.getRules.and.returnValue(of([
      makeRule('PSL', 'r1', 1.0),
      makeRule('PSL', 'r2', 4.0)
    ]));
    ...
    // maxWeight = 4.0, so the scale goes relative and the strongest rule fills the bar.
    expect(component.weightBarPct(component.allRules[1])).toBeCloseTo(1.0);
    expect(component.weightBarPct(component.allRules[0])).toBeCloseTo(0.25);
  });
