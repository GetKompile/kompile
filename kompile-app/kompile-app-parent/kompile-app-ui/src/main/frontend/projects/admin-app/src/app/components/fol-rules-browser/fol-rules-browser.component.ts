...
export class FolRulesBrowserComponent implements OnChanges {
  ...
  /**
   * Denominator for the weight bar: the largest finite rule weight, floored at 1.0. The floor is
   * what keeps the bar on an absolute scale while every rule sits at or below the PSL reference
   * weight, so a set of uniformly weak rules is not stretched to make its best member look full.
   */
  maxWeight = 1.0;
