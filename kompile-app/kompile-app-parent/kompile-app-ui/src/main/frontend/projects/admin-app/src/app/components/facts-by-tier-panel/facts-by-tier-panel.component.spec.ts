...
describe('FactsByTierPanelComponent', () => {
  ...
  afterEach(() => {
     ...
     *
     * The drained body has to be a shape the subscriber can actually consume: /facts does
     * `[...data]`, so flushing `{}` into it throws "data is not iterable" from inside this hook.
     * Jasmine reports a hook throw as an error in afterAll and karma ends the run there, which
     * took down all 1711 admin specs, not just this file's.
     */
    httpMock.match(() => true).forEach(r => r.flush(r.request.url.includes('/facts') ? [] : {}));
    httpMock.verify();
  });
