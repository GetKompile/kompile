...
describe('JobHistoryComponent – integration DOM tests', () => {
  ...
  beforeEach(async () => {
    ...
    /*
     * The step panel injects UnifiedCrawlService. Unstubbed it comes from the root injector
     * and asks for HttpClient, which this TestBed has no provider for — so the failure lands
     * in beforeEach and takes every spec in the file with it.
     */
    const unifiedCrawlSpy = jasmine.createSpyObj('UnifiedCrawlService', [
      'getJobFromHistory',
      'runStep'
    ]);
    unifiedCrawlSpy.getJobFromHistory.and.returnValue(of({ steps: [] }));
    unifiedCrawlSpy.runStep.and.returnValue(of({}));
