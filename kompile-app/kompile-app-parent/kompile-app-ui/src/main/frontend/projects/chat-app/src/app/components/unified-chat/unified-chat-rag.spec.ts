...
describe('UnifiedChatComponent - RAG End-to-End', () => {
  ...
  describe('cancelStreaming()', () => {
    ...
    it('should mark last message as not streaming and flag it stopped', () => {
      component.messages = [{
      ...
      // The cancellation marker is a template badge driven by `stopped`, not text spliced into the
      // message body — see the [data-testid=stopped-badge] span in unified-chat.component.html.
      // Keeping it out of the content is what lets the message be copied or exported cleanly.
      expect(lastMsg.stopped).toBeTrue();
      expect(lastMsg.content).toBe('Partial response');
    });

    it('should strip a trailing [Stopped] the service appended instead of leaving it in the content', () => {
      component.messages = [{
      ...
      // Content is normalised whichever side produced the marker, so the badge can never end up
      // rendered next to a literal duplicate of itself.
      expect(component.messages[0].content).toBe('Partial');
      expect(component.messages[0].stopped).toBeTrue();
    });
  });
  ...
  describe('Session management', () => {
    ...
    it('updateCurrentSession should auto-name from first user message', () => {
      ...
      // sanitizeSessionTitle() only truncates past 60 chars, so a 44-char question is kept whole.
      expect(component.currentSession?.name).toBe('How do I configure vector search in Kompile?');
    });
