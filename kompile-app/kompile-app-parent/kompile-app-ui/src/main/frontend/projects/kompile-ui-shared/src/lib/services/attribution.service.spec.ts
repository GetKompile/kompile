...
describe('AttributionService', () => {
  ...
  describe('explainQuick()', () => {
    it('should GET /attribution/explain?nodeId= with default params', (done) => {
      ...
      const req = httpMock.expectOne(r =>
        r.url === `${apiBase}/explain` && r.params.get('nodeId') === 'node-1' && r.method === 'GET'
      );
      req.flush(mockResult);
    });
    ...
    it('should pass all optional query params', (done) => {
      ...
      const req = httpMock.expectOne(r =>
        r.url === `${apiBase}/explain` &&
        r.params.get('nodeId') === 'node-1' &&
    ...
    /*
     * The slash is the whole point. Document node IDs embed filesystem paths, and Tomcat
     * rejects an encoded %2F inside a URL *path* — which is why the endpoint takes nodeId as
     * a query param (see EventAttributionController.explainQuick). So what this asserts is
     * that the id survives intact as a param, not that it gets encoded into a path segment.
     */
    it('should carry a slash-bearing nodeId as a query param', (done) => {
      const mockResult = {
      ...
      const req = httpMock.expectOne(r =>
        r.url === `${apiBase}/explain` && r.params.get('nodeId') === 'node/special'
      );
      req.flush(mockResult);
    });
  });
  ...
  describe('predictQuick()', () => {
    it('should GET /attribution/predict?nodeId= with default params', (done) => {
      ...
      const req = httpMock.expectOne(r =>
        r.url === `${apiBase}/predict` && r.params.get('nodeId') === 'node-1' && r.method === 'GET'
      );
      req.flush(mockResult);
    });
