...
describe('BayesianService', () => {
  ...
  describe('networkStats()', () => {
    it('should GET /bayesian/network/stats?nodeId=', (done) => {
  ...
  describe('mebnStructure()', () => {
    it('should GET /bayesian/mebn/structure?nodeId=', (done) => {
      ...
      const req = httpMock.expectOne(r =>
        r.url === `${apiBase}/mebn/structure` && r.params.get('nodeId') === 'node-1'
      );
      expect(req.request.method).toBe('GET');
      req.flush(mockResult);
    });
  ...
  describe('mebnStats()', () => {
    it('should GET /bayesian/mebn/stats?nodeId=', (done) => {
      ...
      const req = httpMock.expectOne(r =>
        r.url === `${apiBase}/mebn/stats` && r.params.get('nodeId') === 'node-1'
      );
      expect(req.request.method).toBe('GET');
      req.flush(mockStats);
    });
