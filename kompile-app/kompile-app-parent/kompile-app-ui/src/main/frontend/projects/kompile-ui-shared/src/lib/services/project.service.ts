...
export interface RestorationInventory {
  catalogFactSheets: number;
  runtimeFactSheets: number;
  catalogSourceConnections: number;
  runtimeSourceConnections: number;
  chatSessions: number;
...
export interface RestorationReadiness {
  projectRoot: string;
  active: boolean;
  ready: boolean;
  activationMode: 'ACTIVE' | 'RESTART_REQUIRED';
...
export interface RestorationResult {
  dryRun: boolean;
  applied: boolean;
  plannedFactSheets: number;
  plannedSourceConnections: number;
  restoredFactSheets: number;
  restoredSourceConnections: number;
  skipped: number;
  warnings: string[];
  readiness: RestorationReadiness;
}
...
export class ProjectService extends BaseService {
  ...
  getRestorationReadiness(): Observable<RestorationReadiness> {
    return this.http.get<RestorationReadiness>(
      `${backendUrl}/projects/current/portability/restoration-readiness`
    );
  }

  getImportedRestorationReadiness(id: string): Observable<RestorationReadiness> {
    return this.http.get<RestorationReadiness>(
      `${backendUrl}/projects/current/portability/jobs/${encodeURIComponent(id)}/restoration-readiness`
    );
  }

  restorePortableCatalogs(dryRun = true): Observable<RestorationResult> {
    return this.http.post<RestorationResult>(
      `${backendUrl}/projects/current/portability/restore`,
      {},
      { params: { dryRun } }
    );
  }
