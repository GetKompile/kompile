...
public class TextEmbeddingStepRunnerFactory implements PipelineStepRunnerFactory {
    public static final String RUNNER_FQCN = "ai.kompile.pipelines.steps.vlm.TextEmbeddingStepRunner";

    @Override
    public String stepTypeName() { return VLMConstants.TEXT_EMBEDDING_TYPE; }

    @Override
    public String getRunnerType() { return RUNNER_FQCN; }

    @Override
    public PipelineStepRunner create() { return new TextEmbeddingStepRunner(); }

    @Override
    public StepSchema getSchema() {
        return StepSchema.builder()
                .name(VLMConstants.TEXT_EMBEDDING_TYPE)
                .runnerClassName(RUNNER_FQCN)
                .description("Text embedding (embed_tokens) for VLM pipeline")
                .parameter(ParameterSchema.builder().name(VLMConstants.PARAM_MODEL_URI)
                        .type(ValueType.STRING).required(true)
                        .description("Path to the embed_tokens SameDiff model").build())
