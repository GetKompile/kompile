...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: qualify-tensor-g3-candidate.sh --candidate-manifest FILE --model QWEN.gguf [--adb FILE] [--device-receipt FILE]"
      exit 0
      ;;
...
[[ -n "$CANDIDATE_MANIFEST" && -n "$MODEL" ]] ||
  fail "--candidate-manifest and --model are required"
...
[[ "$serial" != emulator-* && "$device_details" == *"usb:"* ]] ||
  fail "qualification requires one physical USB adb transport: ${device_rows[0]}"
...
[[ "$board_platform" == zumapro ]] || fail "physical device platform is not Tensor G3/zumapro: $board_platform"
...
remote_model="/data/local/tmp/tensor-g3-qualification-$model_sha256.gguf"
...
cleanup() {
  ...
  if [[ -n "$remote_model" ]]; then
    "$ADB" -s "$serial" shell rm -f "$remote_model" >/dev/null 2>&1 || true
  fi
}
...
"$ADB" -s "$serial" shell chmod 0644 "$remote_model" ||
  fail "could not make staged qualification model readable"
