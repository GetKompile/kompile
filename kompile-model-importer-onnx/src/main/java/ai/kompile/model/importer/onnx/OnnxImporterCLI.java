...
/**
 * Standalone CLI tool for converting ONNX models to SameDiff format.
 * This tool is designed to be used as middleware for model conversion without
 * embedding the heavy ONNX import framework in the main application.
 */
@CommandLine.Command(
    name = "onnx-importer",
    description = "Convert ONNX models (.onnx files) to SameDiff format",
    mixinStandardHelpOptions = true,
    version = "1.0.0"
)
@Slf4j
public class OnnxImporterCLI implements Callable<Integer> {

    @CommandLine.Parameters(
        index = "0", 
        description = "Input ONNX model file (.onnx)"
    )
    ...
    @CommandLine.Option(
        names = {"--model-info"},
        description = "Display detailed model information before conversion"
    )
    ...
    public Integer call() throws Exception {
        ...
        log.info("ONNX Model Importer CLI v1.0.0");
        ...
        if (!inputFile.getName().toLowerCase().endsWith(".onnx")) {
            log.warn("Input file does not have .onnx extension: {}", inputFile.getName());
        }
    ...
    private void displayModelInfo() {
        try {
            log.info("Loading ONNX model for inspection...");
            OnnxFrameworkImporter importer = new OnnxFrameworkImporter();
            
            // Load the ONNX graph for inspection
            var onnxGraph = importer.loadGraph(inputFile.getAbsolutePath());
            
            log.info("ONNX Model Information:");
            log.info("  Input count: {}", onnxGraph.getInputList().size());
            log.info("  Output count: {}", onnxGraph.getOutputList().size());
            
            log.info("  Inputs:");
            for (int i = 0; i < onnxGraph.getInputList().size(); i++) {
                String inputName = onnxGraph.inputAt(i);
                var shape = onnxGraph.shapeOfInput(inputName);
                var dtype = onnxGraph.dataTypeForVariable(inputName);
                log.info("    {}: {} ({})", inputName, 
                    shape != null ? java.util.Arrays.toString(shape) : "unknown shape", 
                    dtype != null ? dtype : "unknown type");
            }
            
            log.info("  Outputs:");
            for (int i = 0; i < onnxGraph.getOutputList().size(); i++) {
                String outputName = onnxGraph.outputAt(i);
                log.info("    {}", outputName);
            }
            
        } catch (Exception e) {
            log.warn("Could not display model info: {}", e.getMessage());
        }
    }

    private void validateInputs(Map<String, INDArray> dynamicVariables) throws IOException {
        log.info("Validating ONNX model structure...");
    ...
    private int performConversion(Map<String, INDArray> dynamicVariables) throws IOException {
        log.info("Starting ONNX to SameDiff conversion...");
