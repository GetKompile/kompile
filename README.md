...
# This writes .kompile/project/open.json, starts model-staging on port 8090,
# writes .mcp.json for AI agents, and brings up the three app processes:
#
#   :8080  admin console      (kompile-app-main)
#   :8081  chat               (kompile-app-chat)          --no-chat to skip
#   :8082  crawl manager      (kompile-app-crawl-manager) --no-crawl-manager to skip
#
# All three read one project's config and data. Each is optional: a box with only
# the admin console installed still opens the project and reports what is missing.
```

Open your browser to **http://localhost:8081** — chat is where you start. The setup wizard
runs on any of the three (it is part of the surface they share) and walks you through:
...
**From the web UI** — the crawl manager at **http://localhost:8082** (Crawl tab → New Crawl Job):
...
# The URL is optional — the CLI routes each API family to the process that owns it.
...
internals, process state, build output, and known secret files are excluded. Secrets are never
portable: the legacy `--include-sensitive-files` compatibility option is rejected, and credentials
must be restored separately from an encrypted secret store or backup after import. Export refuses a
running project unless `--allow-running` is explicit. Export reads every included file through
...
# 2. Server-connected RAG chat (chat app, :8081)
kompile chat --url=http://localhost:8081
...
The web application generated by `kompile build app` — a full-stack Spring Boot + Angular
system, split into **three processes** so that end-user surfaces and operator surfaces are
separated by more than a menu. There is no login: what a process mounts is what anyone who
can reach its port can call, so the split is the access boundary. Point end users at :8081
and :8082; keep :8080 to yourself.

**Chat — http://localhost:8081** (`kompile-app-chat`)
...
| **Project** | Browse the open project's files and folders alongside the conversation |
| **Fact Sheets** | Notebook-style knowledge organization, read-only |
| **Graph** | Entity/relationship exploration and visualization, read-only |

**Crawl manager — http://localhost:8082** (`kompile-app-crawl-manager`)

| Screen | What it does |
|--------|-------------|
| **Crawl** (default) | Document ingestion from 20+ sources, live pipeline counters, graph extraction config, per-document status |
| **Fact Sheets** | Full CRUD over fact sheets |
| **Data** | Documents and index browser, chunk inspection, note sync and Connected Services, and **Code Projects** — register repositories, trigger semantic indexing, browse code graphs |
| **Graph** | Entity/relationship browsing and visualization |

**Admin console — http://localhost:8080** (`kompile-app-main`)

| Screen | What it does |
|--------|-------------|
| **Developer** (default) | ND4J framework status, GPU lifecycle, subprocess logs, operation timing, benchmarks, VLM orchestration, SameDiff graph visualization, model debug |
| **Agents** (KClaw) | Run CLI agents interactively in the browser with MCP tool injection, permission management, heartbeat monitoring, session history |
| **Enforcer** | Guardrail and policy enforcement, evaluation sets, eval debugging |
| **Fact Sheets**, **Graph** | Shared with the end-user apps; graph *maintenance* — knowledge-graph console, grounding console, graph simulator — is reachable here only |

**Settings** is not a tab on any of the three: it opens from the gear in the shell header, and
only the admin console routes it. Vector store backend, chunking strategy, embedding config, LLM
provider, query rewriting, reranking, filter chains, guardrails, system prompts, tool gateway
rules, MCP servers, ND4J environment tuning.
