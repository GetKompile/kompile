...
CONFIG_B64='__Kompile_WORKER_CONFIG_B64__'
BUILD_DRIVER_B64='__Kompile_BUILD_DRIVER_B64__'
LOG_FORWARDER_B64='__Kompile_LOG_FORWARDER_B64__'
CONFIG_FILE=/tmp/kompile-release-worker.json
BUILD_DRIVER=/tmp/kompile-build-platform.py
LOG_FORWARDER=/tmp/kompile-log-forwarder.py
LOG_FORWARDER_STOP=/tmp/kompile-log-forwarder.stop
LOG_FORWARDER_ERROR=/tmp/kompile-log-forwarder.err
...
upload_if_present() {
  [ ! -e "$1" ] && return 0
  aws --region "${REGION}" s3 cp "$1" "${S3_PREFIX}/$2" --only-show-errors
}
...
finish() {
  ...
  # status.json is the final durable marker consumed by the serial controller.
  upload_if_present "${OUTPUT_DIR}/status.json" status.json || true
  sync
  shutdown -h now || true
  exit "${exit_code}"
}
...
if [ "${OS_NAME}" = "linux" ]; then
  ...
  brew install ant autoconf autoconf-archive automake awscli binutils bison ccache cmake flex gcc git gmp gnu-sed gradle isl jq libmpc libomp libtool libusb llvm maven mpfr nasm ninja openblas openjdk@11 openjdk@17 perl pkg-config python@3.12 ragel rust sdl swig unzip wget xz || true
...
if [ -n "${CONTAINER_IMAGE}" ]; then
  if [ "${CONTAINER_FAMILY}" = almalinux ]; then
    docker build --tag kompile-release-compat --file "${SOURCE_DIR}/.github/actions/build-centos/Dockerfile" "${SOURCE_DIR}/.github/actions/build-centos"
    build=(docker run --rm --network host -e GITHUB_WORKSPACE=/github/workspace -e LIBND4J_FILE_NAME= -e LIBND4J_URL= -e OPENBLAS_PATH=/usr -e "INSTALL_COMMAND=python3 /kompile-build-platform.py --config /kompile-config.json --source /github/workspace --repository /kompile-m2 --maven-output /kompile-output/maven-repository --sdk-output /kompile-output/sdk-assets" -v "${SOURCE_DIR}:/github/workspace" -v "${MAVEN_REPO}:/kompile-m2" -v "${OUTPUT_DIR}:/kompile-output" -v "${CONFIG_FILE}:/kompile-config.json:ro" -v "${BUILD_DRIVER}:/kompile-build-platform.py:ro" kompile-release-compat)
  else
    build=(docker run --rm --network host -v "${SOURCE_DIR}:/workspace" -v "${MAVEN_REPO}:/kompile-m2" -v "${OUTPUT_DIR}:/kompile-output" -v "${CONFIG_FILE}:/kompile-config.json:ro" -v "${BUILD_DRIVER}:/kompile-build-platform.py:ro" -v /opt/protobuf:/opt/protobuf:ro -v /opt/cmake:/opt/cmake:ro -w /workspace "${CONTAINER_IMAGE}" bash -lc "apt-get update && apt-get install -y --no-install-recommends autoconf automake build-essential ca-certificates ccache cmake gfortran git libomp-dev libopenblas-dev libtool maven nasm ninja-build openjdk-11-jdk pkg-config python3 swig unzip xz-utils zip && export PATH=/opt/protobuf/bin:/opt/cmake/bin:\$PATH && python3 /kompile-build-platform.py --config /kompile-config.json --source /workspace --repository /kompile-m2 --maven-output /kompile-output/maven-repository --sdk-output /kompile-output/sdk-assets")
  fi
fi
setsid "${build[@]}" &
BUILD_PID=$!
...
c=json.load(open(sys.argv[2])); json.dump({"schemaVersion":1,"runId":c["runId"],"shard":c["shard"]["id"],"commit":c["commit"],"dl4jCommit":c["dl4jCommit"],"releaseVersion":c["releaseVersion"],"workloads":c["shard"]["workloads"],"os":c["shard"]["os"],"platform":c["shard"]["build"]["javacppPlatform"],"backend":c["shard"]["build"]["backend"],"files":files},open(root/"shard-manifest.json","w"),indent=2,sort_keys=True)' "${OUTPUT_DIR}" "${CONFIG_FILE}"
