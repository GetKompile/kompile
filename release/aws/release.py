#!/usr/bin/env python3
"""Provision and stop ephemeral Kompile release builders using the standard AWS SDK chain."""
...
MANAGED_TAG = "KompileReleaseManaged"
PROJECT_TAG = "KompileReleaseProject"
RUN_TAG = "KompileReleaseRun"
SHARD_TAG = "KompileReleaseShard"
ACTIVE_STATES = ("pending", "running", "stopping", "stopped")
GPU_INSTANCE_PREFIXES = ("g", "p", "inf", "trn")
DISTRIBUTION_VARIANTS = {
    "cli-only", "full", "hosted", "cpu-intel", "cpu-arm", "cuda", "amd-zluda",
}
KOMPILE_NATIVE_TARGETS = {
...
def prompt_value(label: str, *, default: str | None = None, secret: bool = False,
...
def configure_aws_credentials(boto3: Any, region: str, initial_problem: str) -> Any:
    ...
    if problem is not None:
        raise aws_configuration_error(
            "AWS rejected the values entered for AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY"
...
def configured_aws_session(region: str | None = None, *, allow_wizard: bool = True) -> tuple[Any, str]:
    ...
    if session is None and (not requested_region or not valid_aws_region(requested_region)):
        ...
        session, session_problem = create_aws_session(boto3, region=requested_region)
    credential_problem = aws_credential_problem(session) if session is not None else (
        session_problem or "the boto3 credential chain could not be loaded"
    )
...
def load_plan(path: Path) -> dict[str, Any]:
    ...
    for shard in plan.get("shards", []):
        ...
        if not workloads or not workloads <= {"maven", "sdk", "distribution", "android"}:
    ...
    coverage = plan.get("buildCoverage", {})
    if set(coverage.get("distributionVariants", [])) != DISTRIBUTION_VARIANTS:
        raise ValueError("buildCoverage.distributionVariants must enumerate every build-dist.sh variant")
    if set(coverage.get("kompileNativeTargets", [])) != KOMPILE_NATIVE_TARGETS:
        raise ValueError("buildCoverage.kompileNativeTargets must enumerate every supported native CLI target")
    if coverage.get("androidArtifact") != "debug-apk":
        raise ValueError("buildCoverage.androidArtifact must match the active GitHub Actions debug APK build")
    if coverage.get("publishingPolicy") != "supplemental-assets-to-existing-canonical-github-release":
        raise ValueError("external publishing must remain supplemental to the canonical GitHub release workflow")
    actual_distribution_variants = {
        variant["name"]
        for shard in plan["shards"] if shard["build"]["kind"] == "distribution"
        for variant in shard["build"]["variants"]
    }
    if actual_distribution_variants != DISTRIBUTION_VARIANTS:
        raise ValueError(
            f"release plan distribution variants drifted: {sorted(actual_distribution_variants)}"
        )
    return plan


def execution_shards(plan: dict[str, Any]) -> list[dict[str, Any]]:
    """Create reusable platform/toolchain lanes; each lane runs its variants serially."""
...
def kill_parameter_name(plan: dict[str, Any]) -> str:
    return str(plan.get("killSwitchParameter", "/kompile/release/kill-switch"))


def log_group_name(plan: dict[str, Any]) -> str:
    return str(plan.get("logGroupName", "/kompile/releases"))
...
def ensure_log_group(logs_client, plan: dict[str, Any]) -> str:
    ...
    try:
        logs_client.create_log_group(
            logGroupName=group,
            tags={MANAGED_TAG: "true", PROJECT_TAG: str(plan.get("projectTag", "kompile-release"))},
        )
...
def emit_cloudwatch_event(logs_client, group: str, stream: str, message: str) -> bool:
    ...
    try:
        logs_client.put_log_events(
            ...
            logEvents=[{"timestamp": int(time.time() * 1000), "message": f"[kompile-controller] {message}"}],
        )
        return True
    except Exception as exc:
...
def delete_s3_log_objects(s3, bucket: str, prefix: str,
                          selected: set[str] | None = None) -> list[dict[str, str]]:
    """Permanently delete every version and delete marker for staged build.log objects."""
    _, ClientError = _boto3()
        ...
        for page in pages:
            ...
            for section in ("Versions", "DeleteMarkers"):
                for item in page.get(section, []):
                    ...
                    if selected and shard not in selected:
                        continue
                    candidates.append({"Key": key, "VersionId": item["VersionId"]})
...
def set_kill_switch(ssm, plan: dict[str, Any], enabled: bool) -> None:
    ssm.put_parameter(
        ...
        Description="Global emergency stop for all managed Kompile release builders",
        Type="String",
    ...
    ssm.add_tags_to_resource(
        ...
        Tags=[{"Key": "KompileReleaseKillSwitch", "Value": "true"}],
    )
...
def release_bucket_name(sts, region: str, explicit: str | None) -> str:
    ...
    return f"kompile-release-{account}-{region}".lower()
...
def ensure_instance_profile(iam, bucket: str, parameter: str, log_group: str) -> str:
    _, ClientError = _boto3()
    role = "KompileReleaseBuilderRole"
    profile = "KompileReleaseBuilderProfile"
    trust = {
        "Version": "2012-10-17",
    ...
    try:
        ...
        if exc.response.get("Error", {}).get("Code") != "NoSuchEntity":
            raise
        iam.create_role(RoleName=role, AssumeRolePolicyDocument=json.dumps(trust), Description="Ephemeral Kompile release builders")
    policy = {
        "Version": "2012-10-17",
        "Statement": [
            {"Effect": "Allow", "Action": ["s3:PutObject", "s3:AbortMultipartUpload", "s3:ListBucket"], "Resource": [f"arn:aws:s3:::{bucket}", f"arn:aws:s3:::{bucket}/*"]},
            {"Effect": "Allow", "Action": "ssm:GetParameter", "Resource": "*", "Condition": {"StringEquals": {"ssm:ResourceTag/KompileReleaseKillSwitch": "true"}}},
            {"Effect": "Allow", "Action": ["logs:CreateLogStream", "logs:PutLogEvents"], "Resource": f"arn:aws:logs:*:*:log-group:{log_group}:log-stream:*"},
        ],
    }
    iam.put_role_policy(RoleName=role, PolicyName="KompileReleaseBuilder", PolicyDocument=json.dumps(policy))
    profile_data = None
    try:
...
def render_user_data(worker: Path, values: dict[str, Any]) -> str:
    ...
    return (worker.read_text(encoding="utf-8")
            .replace("__Kompile_WORKER_CONFIG_B64__", encoded)
            .replace("__Kompile_BUILD_DRIVER_B64__", driver)
            .replace("__Kompile_LOG_FORWARDER_B64__", forwarder))


def bootstrap_user_data(os_name: str, url: str) -> str:
    if os_name == "windows":
        return ("<powershell>\n$ErrorActionPreference='Stop'\n"
                "Write-Output '[kompile-phase] phase=cloud-init status=started'\n"
                f"Invoke-WebRequest -UseBasicParsing -Uri '{url}' -OutFile C:\\kompile-worker.ps1\n"
                "Write-Output '[kompile-phase] phase=worker-download status=complete'\n"
                "& powershell.exe -NoProfile -ExecutionPolicy Bypass -File C:\\kompile-worker.ps1\n</powershell>\n")
    return ("#!/usr/bin/env bash\nset -Eeuo pipefail\n"
            "export HOME=${HOME:-/root}\n"
            "export PATH=${PATH:-/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin}\n"
            "printf '[kompile-phase] phase=cloud-init status=started\\n'\n"
            f"curl --fail --location --retry 5 '{url}' -o /tmp/kompile-worker.sh\n"
            "printf '[kompile-phase] phase=worker-download status=complete\\n'\n"
            "chmod 700 /tmp/kompile-worker.sh\nexec /tmp/kompile-worker.sh\n")
...
def managed_instances(ec2, run_id: str | None = None,
...
def apply_execution_overrides(executions: list[dict[str, Any]], instance_type: str | None,
    ...
    if instance_type:
        ...
        if any(item.get("dedicatedHost") for item in executions):
            raise SystemExit("--instance-type cannot override an EC2 Mac dedicated-host lane")
        for item in executions:
    ...
    for lane in executions:
        ...
        if not candidates:
            available = sorted({int(item.get("VCpuInfo", {}).get("DefaultVCpus", 0))
                                ...
                                if expected_architecture in item.get("ProcessorInfo", {}).get("SupportedArchitectures", [])})
            failures.append(
                f"{lane['id']}: no {family} {expected_architecture} size <= {max_cores} vCPUs; family sizes={available}"
            )
            continue
        ...
        build = lane["build"]
        selected_threads = min(int(build.get("buildThreads", selected_cores)), selected_cores)
        selected_heap = min(int(build.get("mavenHeapGiB", 16)), max(2, memory_gib // 2))
...
def stream_lane_logs(logs_client, group: str, stream: str,
...
def describe_instance_eventually(ec2, instance_id: str, timeout_seconds: int = 300) -> dict[str, Any]:
    ...
    while True:
        try:
            ...
            if instances:
                return instances[0]
            error_code, error = "EmptyDescribeInstancesResult", None
        except Exception as exc:
        ...
        print(f"[controller] EC2 instance {instance_id} not visible yet ({error_code}); "
              f"retrying in {int(wait_seconds)}s", flush=True)
        time.sleep(wait_seconds)
        delay = min(delay * 2, 30)


def print_s3_build_log(s3, bucket: str, status_key: str, shard_id: str,
                       max_bytes: int = 256 * 1024) -> bool:
...
def wait_for_lane(ec2, s3, ssm, logs_client, plan: dict[str, Any], instance_id: str,
                  bucket: str, status_key: str, shard_id: str, log_group: str,
                  log_stream: str) -> None:
    ...
    while True:
        ...
        if state != last_state:
            ...
            emit_cloudwatch_event(
                ...
                f"phase=ec2-state status={state} shard={shard_id} instance={instance_id} elapsedSeconds={elapsed}{suffix}",
            )
            last_state = state
        try:
            ...
            if health != last_health:
                ...
                emit_cloudwatch_event(
                    logs_client, log_group, log_stream,
                    f"phase=ec2-health status=changed shard={shard_id} instance={instance_id} "
                    f"instanceCheck={health[0]} systemCheck={health[1]}",
                )
                last_health = health
        except Exception as exc:
...
def preflight(args: argparse.Namespace) -> None:
    ...
    session, region, ec2, ssm, _, sts, _ = session_clients(
        args.region, allow_wizard=not getattr(args, "no_wizard", False)
    )
    executions = selected_executions(plan, args.shard)
    apply_plan_defaults(plan, executions)
    apply_execution_overrides(
        executions, getattr(args, "instance_type", None), getattr(args, "build_threads", None)
    )
    if getattr(args, "instance_type", None) and getattr(args, "max_cores", None) is not None:
        raise SystemExit("--instance-type and --max-cores are mutually exclusive")
    core_schedule = apply_core_constraint(ec2, executions, getattr(args, "max_cores", None))
    instance_types = sorted({item["instanceType"] for item in executions})
    ...
    amis, descriptions = validate_launch_matrix(ec2, ssm, executions, region, default_az)
    offerings: dict[str, list[str]] = {}
    for instance_type in instance_types:
        response = ec2.describe_instance_type_offerings(
            LocationType="availability-zone",
            Filters=[{"Name": "instance-type", "Values": [instance_type]},
                     {"Name": "location", "Values": availability_zones}],
        )
        offerings[instance_type] = sorted(item["Location"] for item in response["InstanceTypeOfferings"])
    required_vcpus = max((
        int(descriptions[item["instanceType"]]["VCpuInfo"]["DefaultVCpus"])
    ...
    try:
        ...
        quota_error = str(exc)
        if getattr(args, "max_cores", None) is not None:
            raise SystemExit(
                f"cannot prove --max-cores schedule is launchable because the EC2 quota could not be read: {exc}"
            ) from exc
    result = {
        "account": sts.get_caller_identity()["Account"], "region": region,
        "serialLanes": len(executions),
        "variantBuilds": sum(len(item["build"]["variants"]) for item in executions),
        "instanceTypes": instance_types,
        "offeringsByAvailabilityZone": offerings, "resolvedAmis": amis,
        "defaultNetwork": {"subnetId": subnet_id, "securityGroupId": group_id,
        ...
        "coreConstraintSchedule": core_schedule or None,
    }
    print(json.dumps(result, indent=2))
    if quota_value is not None and required_vcpus > quota_value:
        raise SystemExit(
            f"Preflight failed: largest serial lane requires {required_vcpus} standard vCPUs but quota is {quota_value:g}"
        )
...
def start(args: argparse.Namespace) -> None:
    ...
    dl4j_commit = args.dl4j_commit or resolve_branch(args.dl4j_repository, args.dl4j_branch or "master")
    session, region, ec2, ssm, s3, sts, iam = session_clients(
        args.region, allow_wizard=not getattr(args, "no_wizard", False)
    )
    logs_client = session.client("logs")
    executions = selected_executions(plan, args.shard)
    apply_plan_defaults(plan, executions)
    if args.dl4j_maven_repository_url and not args.dl4j_sdk_assets_url:
        backend_distributions = [
            shard["id"]
            for shard in executions
            if shard["build"].get("kind") == "distribution"
            and any(
                variant.get("name") not in {"cli-only", "hosted"}
                for variant in shard["build"].get("variants", [])
            )
        ]
        if backend_distributions:
            raise SystemExit(
                "--dl4j-sdk-assets-url is required with repository-backed backend "
                f"distributions: {backend_distributions}"
            )
    apply_execution_overrides(
        executions, getattr(args, "instance_type", None), getattr(args, "build_threads", None)
    )
    ...
    resolved_amis, descriptions = validate_launch_matrix(
        ec2, ssm, executions, region, availability_zone
    )
    peak_vcpus = max((
        int(descriptions[item["instanceType"]]["VCpuInfo"]["DefaultVCpus"])
        for item in executions if not item.get("dedicatedHost")
    ), default=0)
    if getattr(args, "max_cores", None) is not None and peak_vcpus > args.max_cores:
    ...
    if quota_value is not None and peak_vcpus > quota_value:
        raise SystemExit(
            f"calculated schedule requires {peak_vcpus} standard vCPUs but account quota is {quota_value:g}"
        )
    if core_schedule:
        print(json.dumps({"maxCoresConstraint": args.max_cores,
                          "coreConstraintSchedule": core_schedule}, indent=2), flush=True)
    ...
    print(json.dumps({
        ...
        "dl4jSourceBranch": args.dl4j_branch or ("master" if not args.dl4j_commit else None),
        "resolvedDl4jCommit": dl4j_commit,
        "dl4jMavenRepositoryUrl": args.dl4j_maven_repository_url,
        "dl4jMavenRepositoryId": args.dl4j_maven_repository_id,
        "dl4jSdkAssetsUrl": args.dl4j_sdk_assets_url,
    ...
    try:
        for shard in executions:
            ...
            config = {
                "bucket": bucket,
                "artifactPrefix": plan.get("artifactPrefix", "kompile/releases"),
                "runId": run_id,
            ...
            worker_key = f"{plan.get('artifactPrefix', 'kompile/releases')}/{run_id}/bootstrap/{shard['id']}-{shard['worker']}"
            ...
            if shard.get("dedicatedHost"):
                host_response = ec2.allocate_hosts(
                    ...
                    TagSpecifications=[{"ResourceType": "dedicated-host", "Tags": [
                        {"Key": "Name", "Value": f"kompile-release-{shard['id']}"},
                        {"Key": MANAGED_TAG, "Value": "true"},
                        {"Key": PROJECT_TAG, "Value": str(plan.get("projectTag", "kompile-release"))},
            ...
            print(f"launched {shard['id']}: {instance_id} ({shard['os']}, {shard['build']['backend']}, CPU compile host)", flush=True)
            status_key = f"{plan.get('artifactPrefix', 'kompile/releases')}/{run_id}/{shard['id']}/status.json"
            wait_for_lane(
                ec2, s3, ssm, logs_client, plan, instance_id, bucket, status_key,
                shard["id"], log_group, config["logStreamName"],
            )
            # status.json is the worker's final durable marker. Terminate and wait before
            # provisioning the next lane so the serial schedule never overlaps quota usage.
            ec2.terminate_instances(InstanceIds=[instance_id])
            ec2.get_waiter("instance_terminated").wait(InstanceIds=[instance_id])
            if shard.get("dedicatedHost"):
                host_id = placement["HostId"]
                try:
                    ec2.release_hosts(HostIds=[host_id])
                    released_hosts.append(host_id)
                except Exception as exc:
                    # EC2 Mac hosts cannot be released before AWS's 24-hour minimum.
                    pending_hosts[host_id] = str(exc)
            print(f"completed {shard['id']}: {instance_id}; launching next lane", flush=True)
    ...
    print(json.dumps({
        ...
        "releasedDedicatedHosts": released_hosts,
        "pendingDedicatedHosts": pending_hosts,
        "logGroup": log_group,
        "logsCommand": f"python3 release/aws/release.py --region {region} logs --run-id {run_id} --follow",
        "shutdownCommand": f"python3 release/aws/release.py --region {region} stop-everything --wait",
    }, indent=2))
...
def show_logs(args: argparse.Namespace) -> None:
    ...
    group = log_group_name(plan)
    execution_ids = (
        {item["id"] for item in selected_executions(plan, args.shard)}
        if args.shard else set()
    )
    since = None
...
def delete_logs(args: argparse.Namespace) -> None:
    ...
    cloudwatch_prefix = None if args.all_runs else f"{args.run_id}/"
    deleted = delete_log_streams(logs_client, group, cloudwatch_prefix, execution_ids)
    bucket = release_bucket_name(sts, region, args.bucket)
    artifact_prefix = plan.get("artifactPrefix", "kompile/releases")
    s3_prefix = f"{artifact_prefix}/" if args.all_runs else f"{artifact_prefix}/{args.run_id}/"
    deleted_s3 = delete_s3_log_objects(s3, bucket, s3_prefix, execution_ids)
    print(json.dumps({
        "region": region,
        "logGroup": group,
        "bucket": bucket,
        "deletedLogStreams": deleted,
        "deletedS3LogObjectVersions": deleted_s3,
        "deletedS3LogObjectVersionCount": len(deleted_s3),
    }, indent=2))
...
def upload_release_assets(release_tag: str, repository: str, files: list[Path]) -> dict[str, list[str]]:
    """Upload only missing assets; an existing name must have the exact same checksum."""
    view = subprocess.run(
    ...
    if view.returncode != 0:
        detail = view.stderr.strip() or "release was not found"
        raise RuntimeError(
            f"canonical GitHub release {release_tag!r} must already exist before external assets "
            f"are attached: {detail}"
        )
    existing = {line.strip() for line in view.stdout.splitlines() if line.strip()}
    uploaded: list[str] = []
    unchanged: list[str] = []
    pending: list[Path] = []
    with tempfile.TemporaryDirectory(prefix="kompile-existing-release-assets-") as temporary:
        ...
        for path in files:
            ...
            subprocess.run(
                ["gh", "release", "download", release_tag, "--repo", repository,
                 "--pattern", path.name, "--dir", str(comparison_dir), "--clobber"],
                check=True,
            )
            current = comparison_dir / path.name
            if not current.is_file() or file_digest(current) != file_digest(path):
                raise RuntimeError(
                    f"GitHub release asset {path.name!r} already exists with different content; "
                    "refusing to overwrite a canonical release asset"
                )
        ...
        if pending:
            ...
            uploaded.extend(path.name for path in pending)
...
def publish_test_repository(
    ...
    metadata_prefix = repository_prefix + ".kompile/"
    s3.upload_file(
...
def safe_release_member(name: str) -> Path:
    relative = Path(name.replace("\\", "/"))
    if relative.is_absolute() or ".." in relative.parts:
        raise RuntimeError(f"unsafe worker archive member: {name}")
    return relative


def extract_worker_archive(source: Path, destination: Path) -> None:
    ...
    if zipfile.is_zipfile(source):
        with zipfile.ZipFile(source) as archive:
            for member in archive.infolist():
                if member.is_dir():
                    continue
                relative = safe_release_member(member.filename)
        ...
        with tarfile.open(source) as archive:
            for member in archive.getmembers():
                if not member.isfile():
                    continue
                relative = safe_release_member(member.name)
                output = destination / relative
                output.parent.mkdir(parents=True, exist_ok=True)
                incoming = archive.extractfile(member)
                if incoming is None:
                    raise RuntimeError(f"unable to read worker archive member: {member.name}")
                with incoming, output.open("wb") as outgoing:
                    shutil.copyfileobj(incoming, outgoing)
    else:
        raise RuntimeError(f"unsupported worker archive: {source}")


def collect_inner_zip_assets(
    archive: Path,
    directory: Path,
    shard: str,
    source_key: str,
    ...
    """Verify a worker SDK archive and surface its ZIPs as direct release assets."""
    extracted = directory / f"extracted-{shard}"
    extract_worker_archive(archive, extracted)
    manifest_path = extracted / "artifacts.json"
    if not manifest_path.is_file():
        raise RuntimeError(
            f"SDK archive for {shard} is missing its root artifacts.json"
        )
    # Complete distribution directories may legitimately contain the upstream
    # DL4J shard's own nested artifacts.json. Only the worker-root manifest
    # attests the outer transport archive.
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if not isinstance(manifest, list):
        raise RuntimeError(f"invalid SDK artifact manifest for {shard}")
    verified: list[tuple[dict[str, Any], Path]] = []
    for record in manifest:
        relative = safe_release_member(str(record.get("path", "")))
        ...
        if (
            ...
            raise RuntimeError(f"SDK artifact verification failed for {shard}/{relative}")
    ...
    for record, path in verified:
        ...
        if output_name in used_names:
            ...
            if existing.is_file() and file_digest(existing) == file_digest(path):
                continue
            output_name = f"{path.stem}-{shard}.zip"
        if output_name in used_names:
            raise RuntimeError(f"duplicate direct release asset name: {output_name}")
        output = directory / output_name
        shutil.copy2(path, output)
        checksum = directory / f"{output_name}.sha256"
        checksum.write_text(f"{file_digest(output)}  {output_name}\n", encoding="ascii")
        used_names.update({output_name, checksum.name})
        common = {"shard": shard, "sourceKey": source_key, "innerPath": record["path"]}
        assets.extend((
            {
...
def collect(args: argparse.Namespace) -> None:
    ...
    prefix = f"{plan.get('artifactPrefix', 'kompile/releases')}/{args.run_id}/"
    paginator = s3.get_paginator("list_objects_v2")
    ...
    executions = selected_executions(plan, args.shard)
    selected = bool(args.shard)
    expected_shards = {item["id"] for item in executions}
    with tempfile.TemporaryDirectory(prefix="kompile-release-collect-") as temporary:
        ...
        for item in objects:
            ...
            if name == "shard-manifest.json":
                ...
                expected_identity = (
                    args.run_id, shard, args.commit, args.dl4j_commit, args.version, planned_workloads[shard],
            ...
            if name == "maven-repository.tar.gz" and "maven" in planned_workloads[shard]:
                seen_outputs[shard].add("maven")
                maven_archives.append(output)
            if name == "sdk-assets.tar.gz":
                seen_outputs[shard].update(planned_workloads[shard] - {"maven"})
                direct_assets = collect_inner_zip_assets(
                    output, directory, shard, item["Key"], used_asset_names,
                )
                if "distribution" in planned_workloads[shard] and not any(
                    Path(asset.get("innerPath", "")).name.startswith("kompile-dist-")
                    and asset["fileName"].endswith(".zip")
                    for asset in direct_assets
                ):
                    raise RuntimeError(
                        f"distribution shard {shard} did not produce a complete kompile-dist ZIP"
                    )
            ...
            if name in {"maven-repository.tar.gz", "sdk-assets.tar.gz", "shard-manifest.json"}:
                ...
                used_asset_names.add(output_name)
        ...
        test_repository_info = None
        central_tool = Path(__file__).resolve().parents[1] / "central/repository.py"
        if maven_archives and central_tool.exists():
        ...
        manifest = {
            ...
            "releaseVersion": args.version,
            "commit": args.commit,
            "dl4jCommit": args.dl4j_commit,
            "region": region,
            "bucket": args.bucket,
            "workloads": sorted(set().union(*planned_workloads.values())),
        ...
        manifest_suffix = re.sub(r"[^A-Za-z0-9._-]+", "-", args.run_id)
        manifest_path = directory / f"release-build-manifest-{manifest_suffix}.json"
        manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        checksum_path = directory / f"{manifest_path.name}.sha256"
        checksum_path.write_text(f"{file_digest(manifest_path)}  {manifest_path.name}\n", encoding="ascii")
        repository = args.github_repository
        upload_files = [directory / item["fileName"] for item in assets]
        upload_files.extend([manifest_path, checksum_path])
        manifest["githubAssetUpload"] = upload_release_assets(
            args.release_tag, repository, upload_files,
        )
...
def stop_everything(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "purge_logs", False):
        ...
        deleted_s3_logs = delete_s3_log_objects(
            s3, bucket, f"{plan.get('artifactPrefix', 'kompile/releases')}/",
        )
    ...
    print(json.dumps({
        ...
        "cancelledSpotRequests": spot_ids,
        "releasedDedicatedHosts": released_hosts,
        "pendingDedicatedHosts": pending_hosts,
        "storagePurged": args.purge_storage,
...
def parser() -> argparse.ArgumentParser:
    ...
    check.add_argument("--max-cores", type=int, help="greedily select the largest compatible size per serial lane within this EC2 vCPU limit")
    ...
    launch.add_argument("--repository", default="https://github.com/GetKompile/kompile.git")
    ...
    launch.add_argument(
        ...
        help="logical Maven repository id used by the worker for dependency resolution",
    )
    launch.add_argument(
        "--dl4j-sdk-assets-url",
        help=(
            "anonymous-read URL template for the matching DL4J sdk-assets archive; "
            "supports {lane}, {variant}, {platform}, {version}, and {snapshotVersion}, "
            "and requires an adjacent URL.sha256 sidecar"
        ),
    )
    dl4j_source = launch.add_mutually_exclusive_group()
    dl4j_source.add_argument("--dl4j-commit")
    dl4j_source.add_argument("--dl4j-branch")
    ...
    launch.add_argument("--max-cores", type=int, help="greedily select the largest compatible size per serial lane within this EC2 vCPU limit")
    ...
    remove_logs.add_argument("--bucket")
    ...
    gather.add_argument("--dl4j-commit", required=True)
    gather.add_argument("--github-repository", default="GetKompile/kompile")
