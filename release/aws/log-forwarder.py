...
def put(region: str, group: str, stream: str, events: list[dict]) -> None:
    if not events:
        return
    with tempfile.TemporaryDirectory(prefix="kompile-log-events-") as temporary:
...
def main() -> None:
    ...
    while True:
        ...
        if now - last_heartbeat >= args.heartbeat_seconds:
            messages.append(f"[kompile-heartbeat] forwardedBytes={offset}")
            last_heartbeat = now
        ...
        if args.stop_file.exists():
            ...
            if not data:
                ...
                send(args.region, args.group, args.stream, ["[kompile-log-forwarder] stream complete"])
                return
