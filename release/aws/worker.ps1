...
$ConfigB64 = '__Kompile_WORKER_CONFIG_B64__'
$BuildDriverB64 = '__Kompile_BUILD_DRIVER_B64__'
$LogForwarderB64 = '__Kompile_LOG_FORWARDER_B64__'
...
function Upload-IfPresent([string]$Path, [string]$Name) {
  if (Test-Path $Path) {
    aws --region $Region s3 cp $Path "$Prefix/$Name" --only-show-errors
...
try {
  ...
  choco install -y --no-progress awscli ccache cmake git maven ninja temurin11 temurin17 python312 7zip msys2 rustup.install visualstudio2022buildtools visualstudio2022-workload-vctools
  ...
  # The serial controller treats status.json as the final durable marker.
