...
public class MatrixGraphRagServiceTest {
    ...
    class LocalSearchTests {
        ...
        void shouldPerformLocalSearchWithEmbeddingModel() {
            ...
            // Called twice per local query: once for context retrieval, once for session entity tracking.
            verify(graphStore, times(2)).findSimilarNodes(eq(DEFAULT_GRAPH_ID), any(INDArray.class), eq(5), eq(0.0));
        }
    ...
    class KParameterTests {
        ...
        void shouldUseSpecifiedKValueForSearch() {
            ...
            // Called twice per local query: once for context retrieval, once for session entity tracking.
            verify(graphStore, times(2)).findSimilarNodes(eq(DEFAULT_GRAPH_ID), any(INDArray.class), eq(15), eq(0.0));
        }
    }
