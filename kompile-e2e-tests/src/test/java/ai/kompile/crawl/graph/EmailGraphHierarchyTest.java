...
class EmailGraphHierarchyTest {
    ...
    /**
     * Shared node-creation logic used by both the 5-arg and 6-arg createNode stubs.
     * Records the node into the in-memory maps by (externalId, NodeLevel) key.
     */
    private GraphNode createNodeInMemory(NodeLevel level, String extId, String title,
                                          String desc, Map<String, Object> meta) {
    ...
    /**
     * Resolves a synthetic node ID produced by registerDocumentNodes to a real UUID nodeId.
     * registerDocumentNodes uses synthetic IDs like "source_crawl:<jobId>" and
     * "document_<sourcePath>" as edge endpoint references in EdgeSpec. We resolve by scanning
     * nodesByExternalIdAndType for a node whose externalId the synthetic ID encodes.
     *
     * The encoding is: level.lowercase() + "_" + externalId, so strip the prefix.
     */
    private String resolveNodeId(String syntheticId) {
        if (syntheticId == null) return null;
        // Try direct lookup (some callers may pass actual nodeIds already)
        GraphNode direct = nodesById.get(syntheticId);
        if (direct != null) return syntheticId;
        // Strip level prefix: "source_", "document_", "entity_", "snippet_"
        for (NodeLevel level : NodeLevel.values()) {
            String prefix = level.name().toLowerCase() + "_";
            if (syntheticId.startsWith(prefix)) {
                String extId = syntheticId.substring(prefix.length());
                GraphNode node = nodesByExternalIdAndType.get(extKey(extId, level));
                if (node != null) return node.getNodeId();
            }
        }
        return null;
    }
    ...
    private KnowledgeGraphService buildGraphService() {
        ...
        // P1: 3-arg factSheet-scoped overload now used by production
        when(gs.getNodeByExternalId(anyString(), any(NodeLevel.class), any()))
                .thenAnswer(inv -> {
                    String extId = inv.getArgument(0);
                    NodeLevel lvl = inv.getArgument(1);
                    return Optional.ofNullable(nodesByExternalIdAndType.get(extKey(extId, lvl)));
        ...
        // ── createNode ──────────────────────────────────────────────────
        // 5-arg legacy overload — delegates to shared createNodeInMemory helper
        when(gs.createNode(any(NodeLevel.class), anyString(), anyString(), anyString(), anyMap()))
                .thenAnswer(inv -> createNodeInMemory(
                        inv.getArgument(0), inv.getArgument(1),
                        inv.getArgument(2), inv.getArgument(3), inv.getArgument(4)));

        // P1/P4: 6-arg factSheet-scoped overload now used by production
        when(gs.createNode(any(NodeLevel.class), anyString(), anyString(), anyString(), anyMap(), any()))
                .thenAnswer(inv -> createNodeInMemory(
                        inv.getArgument(0), inv.getArgument(1),
                        inv.getArgument(2), inv.getArgument(3), inv.getArgument(4)));
        ...
        // P2/P4: 9-arg createEdgeWithMetadata now used by production — record into in-memory model.
        // Guard against duplicates: production relies on the underlying graph store to deduplicate;
        // the test stub mirrors that by skipping edges where src+tgt already exist.
        when(gs.createEdgeWithMetadata(anyString(), anyString(), any(EdgeType.class),
        ...
        // P3/P4: createNodesBatch — registerDocumentNodes now batches SOURCE+DOCUMENT creation
        when(gs.createNodesBatch(anyList(), any()))
                .thenAnswer(inv -> {
                    List<KnowledgeGraphService.NodeSpec> specs = inv.getArgument(0);
                    List<GraphNode> result = new ArrayList<>();
                    for (KnowledgeGraphService.NodeSpec spec : specs) {
                        GraphNode node = createNodeInMemory(
                                spec.nodeType(), spec.externalId(),
                                spec.title(), spec.description(), spec.metadata());
                        result.add(node);
                    }
                    return result;
                });

        // P3/P4: createEdgesBatch — registerDocumentNodes now batches edge creation.
        // The EdgeSpec carries synthetic string IDs for source/target computed by registerDocumentNodes
        // as "source_<crawlSource>" and "document_<sourcePath>". We need to resolve those to
        // real UUID nodeIds by matching against externalIds in the in-memory store.
        when(gs.createEdgesBatch(anyList()))
                .thenAnswer(inv -> {
                    List<KnowledgeGraphService.EdgeSpec> specs = inv.getArgument(0);
                    int created = 0;
                    for (KnowledgeGraphService.EdgeSpec spec : specs) {
                        // Resolve synthetic IDs: find node whose externalId maps to the synthetic key.
                        // registerDocumentNodes uses "source_crawl:<jobId>" and "document_<sourcePath>".
                        // Strip the level prefix to recover the externalId used in NodeSpec.
                        String srcId = resolveNodeId(spec.sourceNodeId());
                        String tgtId = resolveNodeId(spec.targetNodeId());
                        if (srcId != null && tgtId != null) {
                            String label = spec.label() != null ? spec.label()
                                    : (spec.description() != null ? spec.description() : "RELATED_TO");
                            addEdge(srcId, tgtId, spec.edgeType(), spec.weight(), label);
                            created++;
                        }
                    }
                    return created;
    ...
    void setUp() throws Exception {
        ...
        // Wire the routing/email collaborators the way the crawl slice does (@Autowired fields).
        CrawlDocumentTracker tracker = new CrawlDocumentTracker();
        GraphPersistenceHelper persistence = new GraphPersistenceHelper();
        injectField(persistence, "knowledgeGraphService", graphService);
        injectField(persistence, "documentTracker", tracker);

        router = new ContentTypeRouter();
        injectField(router, "knowledgeGraphService", graphService);
        injectField(router, "graphPersistenceHelper", persistence);
        injectField(router, "documentTracker", tracker);

        extractor = new EmailGraphExtractor();
        injectField(extractor, "knowledgeGraphService", graphService);
        injectField(extractor, "graphPersistenceHelper", persistence);
        injectField(extractor, "documentTracker", tracker);
    }

    private static UnifiedCrawlJob ingestJob(String taskId) {
    ...
    private List<Document> invokeRouteByContentType(String taskId, List<Document> docs) throws Exception {
        return router.routeByContentType(ingestJob(taskId), docs);
    }

    private void invokeEmailGraphExtraction(String taskId, List<Document> docs) throws Exception {
        extractor.applyEmailGraphExtraction(ingestJob(taskId), docs);
    }
    ...
    class DocumentNodeRegistration {
        ...
        void createsSourceAndDocumentNodes() throws Exception {
            ...
            // P5: SOURCE externalId is now "crawl:<jobId>" (was "ingest:<jobId>")
            assertEquals("crawl:task-100", sources.get(0).getExternalId());
    ...
    class DocumentContainsEmail {
        ...
        void containsEdge() throws Exception {
            ...
            // P2: production uses semanticRelationLabel("CONTAINS") = "CONTAINS" as the label,
            // not "EMAIL_MESSAGE". Filter by EdgeType.CONTAINS + DOCUMENT→emailNode direction.
            List<TestEdge> contains = edges.stream()
                    .filter(e -> e.edgeType == EdgeType.CONTAINS
                            && e.sourceNodeId.equals(docNode.getNodeId())
                            && e.targetNodeId.equals(emailNode.getNodeId()))
                    .collect(Collectors.toList());
    ...
    class FullHierarchy {
        ...
        void fullEmailGraph() throws Exception {
            ...
            // P5: EmailGraphExtractor now creates an ORGANIZATION node for the sender's domain
            // (alice@corp.com → org:corp.com). Only the sender's domain creates an ORG entity.
            // 1+1 = 2 (SOURCE+DOCUMENT) + 1 EMAIL + 4 PERSON + 2 ATT + 1 ORG = 10 total
            assertEquals(1, entitiesByMetadataType("ORGANIZATION").size(), "1 ORGANIZATION (corp.com)");
            assertEquals(10, nodesById.size(), "Total node count");
            ...
            // P5: +1 BELONGS_TO edge from sender PERSON to ORG
            assertEquals(1, edgesByLabel("BELONGS_TO").size());
            assertEquals(9, edges.size(), "Total edge count");
    ...
    class LlmExtractedEntities {
        ...
        void llmEntitiesStoredInGraph() throws Exception {
            ...
            // P5: EmailGraphExtractor creates an ORGANIZATION entity for the sender's domain
            // Email: 1 EMAIL_MESSAGE + 2 PERSON + 1 ATTACHMENT + 1 ORGANIZATION(corp.com) = 5
            // LLM: PROJECT + BUDGET + PERSON(John) + EVENT + DEPARTMENT = 5
            // Total ENTITY = 10
            assertEquals(10, nodesByLevel(NodeLevel.ENTITY).size(), "Total ENTITY nodes");
