 ...
 * All tests use synthetic document metadata — no dataset-specific content.
 */
...
class CrossDocumentRelationExtractorTest {
    ...
    @Test
    void fuzzyMatchesAttachmentNames() {
        Map<String, Object> emailMeta = new HashMap<>();
        emailMeta.put("email.subject", "Updated report");
        emailMeta.put("email.attachmentNames", List.of("report_v3_FINAL.xlsx"));

        Document email = makeDoc("message.html", emailMeta);
        // The actual file has a slightly different name (no _FINAL suffix)
        Document attachment = makeDoc("report_v3.xlsx", null);

        stubNodeLookup("/uploads/message.html", "node-email");
        stubNodeLookup("/uploads/report_v3.xlsx", "node-attachment");

        int result = extractor.extractRelations(List.of(email, attachment), 1L);

        assertTrue(result >= 1, "Should fuzzy-match attachment name without _FINAL suffix");
    }

    @Test
    void detectsProcessToDataReferences() {
        // A process document that mentions a sheet name from a spreadsheet
    ...
    void allEdgesUseUserDefinedType() {
        ...
        // Verify all created edges use USER_DEFINED (no hardcoded enums)
