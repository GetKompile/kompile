...
# Kompile Tool Mandate — opencode agent override

You are a opencode agent operating under kompile orchestration.
...
# BANNED CODE PATTERNS — DL4J / DSP / VLM

These apply to ALL code that touches SameDiff, DSP, VLM pipelines, or CUDA graph capture/replay. These are **ABSOLUTE RULES**.

| Pattern | Why | Use Instead |
|---|---|---|
| Hardcoding `GraphExecutionMode` (e.g. forcing `SLOT_BY_SLOT`) | NEVER call `setGraphExecutionMode(GraphExecutionMode.SLOT_BY_SLOT)` or any other forced mode as a workaround. This prevents DSP from advancing through its lifecycle (SLOT_BY_SLOT → SHAPES_FROZEN → REPLAYING), destroys CUDA graph capture/replay, and causes 5-10x performance regressions. | Let DSP manage its own execution mode lifecycle. Fix capture/replay bugs directly — do NOT force SLOT_BY_SLOT to avoid them. |
| Using `StaticKvCacheDecodeLoop` | `StaticKvCacheDecodeLoop` is NOT a fix for anything. It is dead code that has never been a correct solution. It hardcodes execution patterns, clears DSP caches mid-inference, and prevents proper CUDA graph replay. | Use the standard decode path in `GenerationPipeline`. Fix bugs there directly. |
| Calling `clearDynamicShapePlanCache()` mid-inference | Destroys compiled plans, frozen shapes, and captured CUDA graphs, forcing expensive recompilation. | Only call at generation boundaries (new prompt, new page) where shapes actually change. |
| Calling `resetSession()` between frames of the same model | Destroys the DSP executor state, preventing shape freezing and graph capture across repeated executions with the same shapes. | Let DSP persist across frames. Use `resetForNextPage()` at page boundaries if needed. |
| Disabling DSP during prefill or any execution phase | Destroys session/executor state that downstream code depends on. | Keep DSP enabled at all times. |

**No workarounds — EVER.** Fix root causes directly. If DSP crashes, fix DSP. If capture fails, fix capture. If replay corrupts memory, fix replay. Do NOT force modes, clear caches, or route around the problem.
